...
_OPTION_META: dict[str, tuple[str, str]] = {
    ...
    "pos1-elmask": (
        "All",
        "Minimum satellite elevation angle (degrees). Satellites below this are excluded.",
    ),
    "pos1-dynamics": (
        "All",
        "Enable receiver dynamics model (velocity/acceleration state estimation).",
    ),
    "pos1-sateph": (
        "All",
        "Satellite ephemeris source. SSR modes (`brdc+ssrapc`, `brdc+ssrcom`) are used for PPP and PPP-RTK.",
    ),
    "pos1-navsys": (
        "All",
    ...
    "pos1-gridsel": (
        "PPP-RTK, VRS",
        "CLAS grid search radius (m). Controls how far from the rover to search for grid-based tropospheric/ionospheric corrections.",
    ),
    "pos1-rectype": (
        "PPP-RTK, VRS",
    ...
    "pos1-posopt6": (
        "PPP-RTK, VRS, SSR2OSR",
        "Ionospheric compensation method for SSR-based processing.",
    ),
    "pos1-posopt7": (
        "PPP-RTK, VRS",
    ...
    "pos1-tidecorr": (
        "PPP, PPP-RTK, VRS",
        "Tidal displacement correction. Option `solid+otl-clasgrid+pole` uses CLAS grid-based ocean tide loading.",
    ),
    # ── positioning.atmosphere ───────────────────────────────────────────────
    "pos1-ionoopt": (
        "All",
        "Ionospheric correction model. PPP uses `dual-freq` or `est-stec`. PPP-RTK uses `est-stec` or `est-adaptive`. RTK typically uses `dual-freq`.",
    ),
    "pos1-tropopt": (
        "All",
        "Tropospheric correction model. PPP/PPP-RTK use `est-ztd` or `est-ztdgrad`. SPP/RTK typically use `saas`.",
    ),
    ...
    "pos2-arthres": (
        "RTK, PPP, PPP-RTK, VRS",
        "LAMBDA ratio test threshold (2nd-best / best). Typical value: 3.0.",
    ),
    "pos2-arthres1": (
        "RTK, PPP, PPP-RTK",
    ...
    "pos2-arelmask": (
        "RTK, PPP-RTK, VRS",
        "Minimum satellite elevation for AR participation (degrees).",
    ),
    "pos2-elmaskhold": (
        "RTK, PPP-RTK, VRS",
    ...
    "pos2-arminamb": (
        "PPP-RTK, VRS",
        "Minimum number of ambiguities required for partial AR attempt.",
    ),
    "pos2-armaxdelsat": (
        "PPP-RTK, VRS",
        "Maximum satellites to exclude during partial AR satellite rotation.",
    ),
    "pos2-arminfixsats": (
        "RTK, PPP-RTK",
        "Minimum DD pairs required for a valid fix. 0 = disabled.",
    ),
    "pos2-armindropsats": (
        "RTK",
        "Minimum DD pairs before excluding the weakest satellite in partial AR. 0 = disabled.",
    ),
    "pos2-arminholdsats": (
        "RTK",
        "Minimum DD pairs for fix-and-hold mode activation. 0 = no minimum.",
    ),
    ...
    "pos2-rejionno": (
        "RTK, PPP",
        "Innovation (pre-fit residual) threshold (m). Observations exceeding this are rejected.",
    ),
    "pos2-rejionno1": ("PPP-RTK, VRS", "L1/L2 residual rejection threshold (\u03c3)."),
    "pos2-rejionno2": (
        "PPP-RTK, VRS",
        "Dispersive (ionospheric) residual rejection threshold (\u03c3).",
    ),
    "pos2-rejionno3": (
        "PPP-RTK, VRS",
        "Non-dispersive (geometric) residual rejection threshold (\u03c3).",
    ),
    ...
    "pos2-rejdiffpse": (
        "VRS",
        "Pseudorange consistency check threshold (m). Rejects satellites with large code-phase disagreement.",
    ),
    "pos2-poserrcnt": ("VRS", "Number of consecutive position error epochs before filter reset."),
    # ── slip_detection ───────────────────────────────────────────────────────
    "pos2-slipthres": (
        "RTK, PPP, PPP-RTK, VRS",
        "Geometry-free (LG) cycle slip detection threshold (m).",
    ),
    "pos2-thresdop": (
        "RTK, PPP, PPP-RTK",
        "Doppler-phase rate cycle slip detection threshold (cyc/s). 0 = disabled.",
    ),
    # ── kalman_filter ────────────────────────────────────────────────────────
    "pos2-niter": (
        "RTK, PPP, PPP-RTK, VRS",
    ...
    "stats-eratio1": (
        "All",
        "Code/phase measurement error ratio for L1. Code error = phase error \u00d7 ratio. Typical: 100.",
    ),
    "stats-eratio2": ("All", "Code/phase measurement error ratio for L2."),
    "stats-eratio3": ("All", "Code/phase measurement error ratio for L5."),
    "stats-errphase": (
        "All",
        "Base carrier phase measurement error (m). Used in elevation-dependent weighting: error = phase + phase_elevation / sin(el).",
    ),
    ...
    "stats-stdbias": (
        "RTK, PPP, PPP-RTK, VRS",
        "Initial standard deviation for carrier phase bias states (m).",
    ),
    "stats-stdiono": (
        "PPP-RTK, VRS",
    ...
    "stats-prnbias": (
        "RTK, PPP, PPP-RTK, VRS",
        "Process noise for carrier phase bias states (m/\u221as).",
    ),
    "stats-prniono": ("PPP-RTK, VRS", "Process noise for ionospheric delay states (m/\u221as)."),
    "stats-prnionomax": (
        "PPP-RTK, VRS",
        "Maximum ionospheric process noise clamp (m). Prevents excessive iono state growth.",
    ),
    "stats-prntrop": (
        "PPP, PPP-RTK, VRS",
        "Process noise for tropospheric delay states (m/\u221as).",
    ),
    "stats-prnaccelh": (
        "All",
        "Horizontal acceleration process noise (m/s\u00b2). Active when `dynamics = true`.",
    ),
    "stats-prnaccelv": (
        "All",
        "Vertical acceleration process noise (m/s\u00b2). Active when `dynamics = true`.",
    ),
    ...
    "pos2-forgetion": (
        "PPP-RTK, VRS",
        "Forgetting factor for ionospheric state (0\u20131). Lower = faster adaptation.",
    ),
    "pos2-afgainion": ("PPP-RTK, VRS", "Adaptive gain for ionospheric process noise adjustment."),
    "pos2-forgetpva": (
        "PPP-RTK, VRS",
        "Forgetting factor for position/velocity/acceleration states (0\u20131).",
    ),
    ...
    "ant1-pos1": (
        "All",
        "Rover position coordinate 1 (latitude or X, depending on `position_type`).",
    ),
    ...
    "out-maxsolstd": (
        "All",
        "Maximum solution standard deviation for output (m). Solutions exceeding this are suppressed.",
    ),
    ...
    "file-eopfile": (
        "PPP, PPP-RTK",
        "Earth orientation parameter file for precise coordinate frame.",
    ),
    "file-blqfile": (
        "PPP, PPP-RTK",
    ...
    "file-cssrgridfile": (
        "PPP-RTK, VRS",
        "CLAS CSSR grid definition file. Required for CLAS grid-based corrections.",
    ),
    ...
    "misc-timeout": (
        "RT",
        "Stream read timeout (ms). After timeout, stream is considered disconnected.",
    ),
    "misc-reconnect": ("RT", "Stream reconnection interval (ms) after disconnect."),
    "misc-nmeacycle": (
        "RT",
        "NMEA request transmission cycle (ms) for NTRIP caster position updates.",
    ),
