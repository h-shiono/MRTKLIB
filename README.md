...
| **v0.7.x** | Global SSR correction services: Galileo HAS (E6-B C/NAV) float PPP; two-receiver QZSS L6 (mosaic-CLAS / mosaic-G5) with single-SBF MADOCA-PPP; CLAS PPP-RTK accuracy/robustness fixes and an upstream-identical CSSR dump (`mrtk dump --parse`); real-time MADOCA-PPP PPP-AR/IONO (L6D wide-area ionosphere) — BeiDou PPP-B2b planned next |

**Latest — v0.7.3:** Real-time MADOCA-PPP **PPP-AR/IONO**. The MADOCA-PPP L6D wide-area ionospheric augmentation (QZS PRN 200/201) is now wired into the real-time `mrtk run` path, so `ionosphere = est-stec` + `iono_correction = on` applies the same slant-TEC constraint in real time as in post-processing — previously this mode was post-processing only and an `est-stec` real-time run silently ran as plain PPP-AR. The Septentrio SBF L6D decoder steers PRN 200/201 (MADOCA ionosphere, disjoint from CLAS 193–199 per IS-QZSS-MDC-004 Table 3-1) to the iono decoder via a dedicated return code that `rtksvr` feeds into `nav.pppiono`, mirroring the post-processing path — so a **single mosaic-G5 SBF stream** carries observations, L6E SSR **and** L6D ionosphere in one slot. Confirmed on **mosaic-G5 hardware**: the real-time iono constraint applies live (L6E SSR age ≈ 1 s) and PPP-AR/IONO reaches a fixed solution ≈ 1 minute after start, holding ~1.2 cm horizontal / ~3.4 cm vertical ([#223](https://github.com/h-shiono/MRTKLIB/issues/223), [PR #233](https://github.com/h-shiono/MRTKLIB/pull/233)).
