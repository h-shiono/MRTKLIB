...
| **v0.7.x** | Global SSR correction services: Galileo HAS (E6-B C/NAV) float PPP; two-receiver QZSS L6 (mosaic-CLAS / mosaic-G5) with single-SBF MADOCA-PPP; CLAS PPP-RTK accuracy/robustness fixes and an upstream-identical CSSR dump (`mrtk dump --parse`); real-time MADOCA-PPP PPP-AR/IONO (L6D wide-area ionosphere); CLAS PPP-RTK claslib parity closing the storm-day fix-rate gap; TOML configuration redesign with deprecation aliases; CLAS dual-channel transmit-pattern merge in post-processing and real-time; kinematic-RTK slip-detection regression fix; `rtcm_t` memory-footprint refactor (lazy `lclblock_t`: ~1.2 GB → tens of MB per real-time solver); RTCM3 MSM BDS-3 B2b observation support (decoder + encoder) — BeiDou PPP-B2b planned next |

**Latest — v0.7.10:** **RTCM3 MSM: BeiDou-3 B2b observations no longer dropped — decoder and encoder.** The BeiDou MSM signal-ID table (`msm_sig_cmp[]`, inherited byte-identical from MALIB) left RTCM signal IDs 25–27 empty; RTCM 10403.3 Amendment 2 assigns them to the BDS-3 B2b signals `7D` / `7P` / `7Z`, so every B2b cell in a live MSM stream was silently discarded by the decoder (only a level-2 trace). The RTCM3 encoder's `to_sigid()` shares the same table, so RTCM3 output re-encoded from SBF / NovAtel / BINEX input — whose decoders already emit B2b — lost it as well. The three-entry table fix repairs both directions ([#333](https://github.com/h-shiono/MRTKLIB/issues/333), reported by @philippebourcier with RENAG / mosaic-X5 field comparisons; [PR #335](https://github.com/h-shiono/MRTKLIB/pull/335)). New `utest_msm_bds_b2b` — the suite's first MSM encode→decode round-trip, with wire-ID assertions on the MSM header mask — fails on the old table and passes on the new one ([#296](https://github.com/h-shiono/MRTKLIB/issues/296)). Slot note: the decoder's per-band code priority (`7D > 7I`) puts `7D` in the B2 slot and moves a co-present BDS-2 `7I` to an extended slot, the same placement the RINEX reader already produces, so RTCM input now matches the RINEX path. No positioning change in the regression suite — no bundled positioning stream carries BDS MSM, so the gate matches the develop baseline (122 tests, one documented environment-only LAPACK-tolerance failure, `madocalib_pppar_ion_check`); positioning use of B2b is tracked in [#189](https://github.com/h-shiono/MRTKLIB/issues/189) / [#234](https://github.com/h-shiono/MRTKLIB/issues/234).

**v0.7.9:** **Memory footprint: a real-time solver in tens of MB instead of ~1.2 GB.** `rtcm_t` embedded the ~307 MB MADOCA local-correction block (`lclblock_t`) by value — paid three times by every `mrtk run` server and twice more per stream converter — while only the local-correction feature (RTCM3 types 2001–2016) ever uses it. The block is now lazily heap-allocated on the first local-correction message: `rtcm_t` 314 MB → 7.5 MB, `rtksvr_t` 972 MB → 51 MB, measured `mrtk run` startup RSS **808 MiB → 84 MiB (−89.5 %)** — memory stops being the binding constraint on solvers per host ([#295](https://github.com/h-shiono/MRTKLIB/issues/295), reported with production measurements from a containerized CLAS deployment; [PR #323](https://github.com/h-shiono/MRTKLIB/pull/323)). Zero positioning change — outside the allocation guards the migration is byte-identical; the regression gate matches the develop baseline (121 tests, one documented environment-only LAPACK-tolerance failure, `madocalib_pppar_ion_check`). Also fixes a latent uninitialized-pointer free in the stream-converter path and adds the first regression coverage of the local-correction message path (`utest_lclblk`).

**v0.7.8:** **Kinematic RTK: rover cycle-slip detection restored.** A v0.6.10 change made the SPP seed overwrite the RTK engine's previous-carrier-phase bookkeeping every epoch, silently disabling rover-side LLI and Doppler cycle-slip detection in relative RTK for six minor releases — undetected slips carried integer-cycle phase-bias errors into fix-and-hold (wrong fixes with centimetre residuals and a 240 m error tail; collapsed fix availability). Root-caused by per-tag bisect and fixed by giving the SPP TDCP snapshot its own private `ssat_t` slots ([#318](https://github.com/h-shiono/MRTKLIB/issues/318), [PR #320](https://github.com/h-shiono/MRTKLIB/pull/320)). **Kinematic RTK returns to the v0.4.1 record on all six PPC benchmark cases** (e.g. nagoya_run2 fix 7.4 % → 28.3 %, tokyo_run2 misfix 11.1 % → 0.6 %); CLAS and MADOCA verified unchanged. Also: benchmark harness fixes — hourly L6 sessions concatenated so hour-crossing drives keep corrections, and a per-tier `<30cm` accuracy column whose FIX-row complement is the misfix rate ([#316](https://github.com/h-shiono/MRTKLIB/issues/316), [PR #317](https://github.com/h-shiono/MRTKLIB/pull/317)).
