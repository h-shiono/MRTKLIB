...
| **v0.7.x** | Global SSR correction services: Galileo HAS (E6-B C/NAV) float PPP; two-receiver QZSS L6 (mosaic-CLAS / mosaic-G5) with single-SBF MADOCA-PPP; CLAS PPP-RTK accuracy/robustness fixes and an upstream-identical CSSR dump (`mrtk dump --parse`) — BeiDou PPP-B2b planned next |

**Latest — v0.7.2:** CLAS PPP-RTK accuracy/robustness fixes and a CSSR dump tool. The PPP-RTK adaptive-ionosphere process noise is restored under storm-day conditions — the compacted→full-`nx` `Qp` scatter-back that had been lost is reinstated, lifting the storm-day CLAS fix rate from 77% to **85%** (+8 pp, [#225](https://github.com/h-shiono/MRTKLIB/issues/225), [PR #226](https://github.com/h-shiono/MRTKLIB/pull/226)). CLAS now **falls back to global orbit/clock corrections** when a close-network partition goes stale instead of dropping corrections ([#218](https://github.com/h-shiono/MRTKLIB/issues/218), [PR #228](https://github.com/h-shiono/MRTKLIB/pull/228)). `mrtk dump` gains a **`--parse`** mode that reproduces the upstream claslib `dumpcssr` per-subtype CSV output (`parse_cssr_type1..12*.csv`, `parse_cssr_header.csv`) **byte-identically** — implemented as a self-contained vendored parser that leaves the production CLAS decoder untouched — with the CLAS grid definition now passed directly via **`-grid FILE`**. CLAS PPP-RTK datasets also gain GSI **F5 absolute-accuracy** regression checks (2D 95% ≈ 4.2–4.3 cm vs geodetic truth) ([PR #230](https://github.com/h-shiono/MRTKLIB/pull/230), [PR #227](https://github.com/h-shiono/MRTKLIB/pull/227)).
