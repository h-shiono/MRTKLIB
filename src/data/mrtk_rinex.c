...
static int sat2code(int sat, char* code) {
    ...
    switch (satsys(sat, &prn)) {
        case SYS_GPS:
            sprintf(code, "G%02d", prn - MINPRNGPS + 1);
            break;
        case SYS_GLO:
            sprintf(code, "R%02d", prn - MINPRNGLO + 1);
            break;
        case SYS_GAL:
            sprintf(code, "E%02d", prn - MINPRNGAL + 1);
            break;
        case SYS_SBS:
            sprintf(code, "S%02d", prn - 100);
            break;
        case SYS_QZS:
            sprintf(code, "J%02d", prn - MINPRNQZS + 1);
            break;
        case SYS_CMP:
            sprintf(code, "C%02d", prn - MINPRNCMP + 1);
            break;
        case SYS_IRN:
            sprintf(code, "I%02d", prn - MINPRNIRN + 1);
            break;
        default:
            return 0;
    }
...
static double uravalue(int sva) { return 0 <= sva && sva < 15 ? ura_nominal[sva] : 8192.0; }
...
static void convcode(int ver, int sys, const char* str, char* type) {
    strcpy(type, "   ");
...
static void decode_obsh(FILE* fp, char* buff, int ver, int* tsys, char tobs[][MAXOBSTYPE][4], nav_t* nav, sta_t* sta) {
...
static int readrnxh(FILE* fp, int* ver, char* type, int* sys, int* tsys, char tobs[][MAXOBSTYPE][4], nav_t* nav,
                    sta_t* sta) {
    char buff[MAXRNXLEN], *label = buff + 60, ssys;
    int i = 0;

    trace(NULL, 3, "readrnxh:\n");
    ...
    *sys = SYS_GPS;
    ...
    while (fgets(buff, MAXRNXLEN, fp)) {
        if (strlen(buff) <= 60) {
            ...
            *ver = (int)(str2num(buff, 0, 9) * 100);
            if (*ver >= 304 && *(buff + 21) == 'C') { /* CLOCK RINEX 3.04 */
                *type = 'C';
                ssys = *(buff + 42);
            } else {
                *type = *(buff + 20);
            ...
            switch (ssys) {
                ...
                case 'M':
                    ...
                    *tsys = TSYS_GPS;
                    break; /* mixed */
                default:
                    trace(NULL, 2, "not supported satellite system: %c\n", *(buff + 40));
                    break;
            }
            continue;
...
static int decode_obsepoch(FILE* fp, char* buff, int ver, gtime_t* time, int* flag, int* sats) {
    int i, j, n;
    char satid[8] = "";
    ...
    trace(NULL, 4, "decode_obsepoch: time=%s flag=%d\n", time_str(*time, 3), *flag);
    return n;
}
/* convert SNR value to 16 bit SNR -------------------------------------------*/
static uint16_t snr_val(int sat, double snr) {
    if (snr < 0.0 || snr >= 100.0) {
        trace(NULL, 3, "invalid SNR value: sat=%d snr=%.3f\n", sat, snr);
        return (uint16_t)0;
    }
    return (uint16_t)(snr / SNR_UNIT + 0.5);
}
/* decode observation data ---------------------------------------------------*/
static int decode_obsdata(FILE* fp, char* buff, int ver, int mask, sigind_t* index, obsd_t* obs) {
    sigind_t* ind;
    ...
    trace(NULL, 4, "decode_obsdata: ver=%.2f\n", ver / 100.0);
    ...
    if (!obs->sat) {
        trace(NULL, 4, "decode_obsdata: unsupported sat sat=%s\n", satid);
        stat = 0;
    ...
    for (i = 0; i < ind->n; i++) {
        ...
        switch (ind->type[i]) {
            ...
            case 2:
                ...
                break;
            case 3:
                obs->SNR[p[i]] = snr_val(obs->sat, val[i]);
                break;
        }
    }
    trace(NULL, 4, "decode_obsdata: time=%s sat=%2d\n", time_str(obs->time, 0), obs->sat);
    return 1;
}
/* save cycle slips ----------------------------------------------------------*/
...
/* set signal index ----------------------------------------------------------*/
static void set_index(int ver, int sys, const char* opt, char tobs[MAXOBSTYPE][4], sigind_t* ind) {
    const char* p;
    char str[8], *optstr = "";
    double shift;
    int i, j, k, n;
...
#if 0 /* for debug */
    for (i=0;i<n;i++) {
        trace(NULL,2,"set_index: sys=%2d,tobs=%s code=%2d pri=%2d idx=%d pos=%d shift=%5.2f\n",
...
static int readrnxobsb(FILE* fp, const char* opt, int ver, int* tsys, char tobs[][MAXOBSTYPE][4], int* flag,
                       obsd_t* data, sta_t* sta) {
    gtime_t time = {0};
...
static int readrnxobs(FILE* fp, gtime_t ts, gtime_t te, double tint, const char* opt, int rcv, int ver, int* tsys,
...
static int decode_eph(int ver, int sat, gtime_t toc, const double* data, eph_t* eph) {
    ...
    trace(NULL, 4, "decode_eph: ver=%.2f sat=%2d\n", ver / 100.0, sat);

    sys = satsys(sat, &prn);

    if (!(sys & (SYS_GPS | SYS_GAL | SYS_QZS | SYS_CMP | SYS_IRN))) {
        trace(NULL, 3, "ephemeris error: invalid satellite sat=%2d\n", sat);
        return 0;
    }
    *eph = eph0;
    ...
    if (sys == SYS_GPS || sys == SYS_QZS) {
        ...
        eph->type = (eph->code & 5) ? 0 : 1;                /* ephemeris type = I/NAV or F/NAV */
    ...
    if (eph->iode < 0 || 1023 < eph->iode) {
        trace(NULL, 2, "rinex nav invalid: sat=%2d iode=%d\n", sat, eph->iode);
    }
    if (eph->iodc < 0 || 1023 < eph->iodc) {
        trace(NULL, 2, "rinex nav invalid: sat=%2d iodc=%d\n", sat, eph->iodc);
    }
    return 1;
}
/* decode GLONASS ephemeris --------------------------------------------------*/
static int decode_geph(int ver, int sat, gtime_t toc, double* data, geph_t* geph) {
    geph_t geph0 = {0};
    gtime_t tof;
    double tow, tod;
    int week, dow;

    trace(NULL, 4, "decode_geph: ver=%.2f sat=%2d\n", ver / 100.0, sat);

    if (satsys(sat, NULL) != SYS_GLO) {
        trace(NULL, 3, "glonass ephemeris error: invalid satellite sat=%2d\n", sat);
        return 0;
    }
    *geph = geph0;
    ...
    tod = (ver <= 299) ? data[2] : fmod(data[2], 86400.0); /* Tod (v.2), Tow (v.3) in UTC */
    ...
    geph->svh = (int)data[6]; /* MSB of B_n */
...
static int decode_seph(int ver, int sat, gtime_t toc, double* data, seph_t* seph) {
    seph_t seph0 = {0};
    int week;

    trace(NULL, 4, "decode_seph: ver=%.2f sat=%2d\n", ver / 100.0, sat);

    if (satsys(sat, NULL) != SYS_SBS) {
        trace(NULL, 3, "geo ephemeris error: invalid satellite sat=%2d\n", sat);
        return 0;
    }
    *seph = seph0;
...
static int readrnxnavb(FILE* fp, const char* opt, int ver, int sys, int* type, eph_t* eph, geph_t* geph, seph_t* seph,
                       nav_t* nav) {
    ...
    trace(NULL, 4, "readrnxnavb: ver=%.2f sys=%d\n", ver / 100.0, sys);
    ...
    while (fgets(buff, MAXRNXLEN, fp)) {
        ...
        if (i == 0) {
            ...
            if (str2time(buff + sp, 0, 19, &toc)) {
                trace(NULL, 2, "rinex nav toc error: %23.23s\n", buff);
                return 0;
            }
            /* decode data fields */
...
static int add_eph(nav_t* nav, const eph_t* eph) {
    ...
    if (nav->nmax <= nav->n) {
        ...
        if (!(nav_eph = (eph_t*)realloc(nav->eph, sizeof(eph_t) * nav->nmax))) {
            trace(NULL, 1, "decode_eph malloc error: n=%d\n", nav->nmax);
            free(nav->eph);
            nav->eph = NULL;
            nav->n = nav->nmax = 0;
            return 0;
...
static int add_geph(nav_t* nav, const geph_t* geph) {
    ...
    if (nav->ngmax <= nav->ng) {
        ...
        if (!(nav_geph = (geph_t*)realloc(nav->geph, sizeof(geph_t) * nav->ngmax))) {
            trace(NULL, 1, "decode_geph malloc error: n=%d\n", nav->ngmax);
            free(nav->geph);
            nav->geph = NULL;
            nav->ng = nav->ngmax = 0;
            return 0;
...
static int add_seph(nav_t* nav, const seph_t* seph) {
    ...
    if (nav->nsmax <= nav->ns) {
        ...
        if (!(nav_seph = (seph_t*)realloc(nav->seph, sizeof(seph_t) * nav->nsmax))) {
            trace(NULL, 1, "decode_seph malloc error: n=%d\n", nav->nsmax);
            free(nav->seph);
            nav->seph = NULL;
            nav->ns = nav->nsmax = 0;
            return 0;
...
static int readrnxnav(FILE* fp, const char* opt, int ver, int sys, nav_t* nav) {
    eph_t eph;
    geph_t geph;
    seph_t seph;
    int stat, type;
    ...
    trace(NULL, 4, "nav n=%d, ng=%d, ns=%d\n", nav->n, nav->ng, nav->ns);

    trace(NULL, 4, "nav utc_gps=%10.3e,%10.3e,%10.3e,%10.3e\n", nav->utc_gps[0], nav->utc_gps[1], nav->utc_gps[2],
          nav->utc_gps[3]);
    trace(NULL, 4, "nav utc_sbs=%10.3e,%10.3e,%10.3e,%10.3e\n", nav->utc_sbs[0], nav->utc_sbs[1], nav->utc_sbs[2],
          nav->utc_sbs[3]);
    trace(NULL, 4, "nav utc_glo=%10.3e,%10.3e\n", nav->utc_glo[0], nav->utc_glo[1]);
    trace(NULL, 4, "nav utc_gal=%10.3e,%10.3e,%10.3e,%10.3e\n", nav->utc_gal[0], nav->utc_gal[1], nav->utc_gal[2],
          nav->utc_gal[3]);
    trace(NULL, 4, "nav utc_qzs=%10.3e,%10.3e,%10.3e,%10.3e\n", nav->utc_qzs[0], nav->utc_qzs[1], nav->utc_qzs[2],
          nav->utc_qzs[3]);
    trace(NULL, 4, "nav utc_cmp=%10.3e,%10.3e,%10.3e,%10.3e\n", nav->utc_cmp[0], nav->utc_cmp[1], nav->utc_cmp[2],
          nav->utc_cmp[3]);
    trace(NULL, 4, "nav utc_irn=%10.3e,%10.3e,%10.3e,%10.3e\n", nav->utc_irn[0], nav->utc_irn[1], nav->utc_irn[2],
          nav->utc_irn[3]);

    trace(NULL, 4, "nav ion_gps=%10.3e,%10.3e,%10.3e,%10.3e,%10.3e,%10.3e,%10.3e,%10.3e\n", nav->ion_gps[0],
          nav->ion_gps[1], nav->ion_gps[2], nav->ion_gps[3], nav->ion_gps[4], nav->ion_gps[5], nav->ion_gps[6],
          nav->ion_gps[7]);
    trace(NULL, 4, "nav ion_gal=%10.3e,%10.3e,%10.3e,%10.3e\n", nav->ion_gal[0], nav->ion_gal[1], nav->ion_gal[2],
          nav->ion_gal[3]);
    trace(NULL, 4, "nav ion_qzs=%10.3e,%10.3e,%10.3e,%10.3e,%10.3e,%10.3e,%10.3e,%10.3e\n", nav->ion_qzs[0],
          nav->ion_qzs[1], nav->ion_qzs[2], nav->ion_qzs[3], nav->ion_qzs[4], nav->ion_qzs[5], nav->ion_qzs[6],
          nav->ion_qzs[7]);
    trace(NULL, 4, "nav ion_cmp=%10.3e,%10.3e,%10.3e,%10.3e,%10.3e,%10.3e,%10.3e,%10.3e\n", nav->ion_cmp[0],
          nav->ion_cmp[1], nav->ion_cmp[2], nav->ion_cmp[3], nav->ion_cmp[4], nav->ion_cmp[5], nav->ion_cmp[6],
          nav->ion_cmp[7]);
    trace(NULL, 4, "nav ion_irn=%10.3e,%10.3e,%10.3e,%10.3e,%10.3e,%10.3e,%10.3e,%10.3e\n", nav->ion_irn[0],
          nav->ion_irn[1], nav->ion_irn[2], nav->ion_irn[3], nav->ion_irn[4], nav->ion_irn[5], nav->ion_irn[6],
          nav->ion_irn[7]);

    return nav->n > 0 || nav->ng > 0 || nav->ns > 0;
}
/* read RINEX clock ----------------------------------------------------------*/
static int readrnxclk(FILE* fp, const char* opt, int ver, int index, nav_t* nav) {
    pclk_t* nav_pclk;
    gtime_t time;
    double data[2];
    int i, j, sat, mask;
    ...
    while (fgets(buff, sizeof(buff), fp)) {
        if (str2time(buff, (ver >= 304 ? 13 : 8), 26, &time)) {
            trace(NULL, 2, "rinex clk invalid epoch: %34.34s\n", buff);
            continue;
        }
        ...
        if (nav->nc >= nav->ncmax) {
            ...
            if (!(nav_pclk = (pclk_t*)realloc(nav->pclk, sizeof(pclk_t) * (nav->ncmax)))) {
                trace(NULL, 1, "readrnxclk malloc error: nmax=%d\n", nav->ncmax);
                free(nav->pclk);
                nav->pclk = NULL;
                nav->nc = nav->ncmax = 0;
                return -1;
...
/* read RINEX file -----------------------------------------------------------*/
static int readrnxfp(FILE* fp, gtime_t ts, gtime_t te, double tint, const char* opt, int flag, int index, char* type,
                     obs_t* obs, nav_t* nav, sta_t* sta) {
    int ver, sys, tsys = TSYS_GPS;
    ...
    trace(NULL, 2, "unsupported rinex type ver=%.2f type=%c\n", ver / 100.0, *type);
    return 0;
}
/* uncompress and read RINEX file --------------------------------------------*/
static int readrnxfile(const char* file, gtime_t ts, gtime_t te, double tint, const char* opt, int flag, int index,
    ...
    if (!(fp = fopen(cstat ? tmpfile : file, "r"))) {
        trace(NULL, 2, "rinex file open error: %s\n", cstat ? tmpfile : file);
        return 0;
    }
    /* read RINEX file */
 ...
 *
 *-----------------------------------------------------------------------------*/
int readrnxt(const char* file, int rcv, gtime_t ts, gtime_t te, double tint, const char* opt, obs_t* obs, nav_t* nav,
             sta_t* sta) {
    int i, n, stat = 0;
    const char* p;
    char type = ' ', *files[MAXEXFILE] = {0};

    trace(NULL, 3, "readrnxt: file=%s rcv=%d\n", file, rcv);
...
int readrnx(const char* file, int rcv, const char* opt, obs_t* obs, nav_t* nav, sta_t* sta) {
    ...
    trace(NULL, 3, "readrnx : file=%s rcv=%d\n", file, rcv);
...
static void combpclk(nav_t* nav) {
    ...
    if (!(nav_pclk = (pclk_t*)realloc(nav->pclk, sizeof(pclk_t) * nav->nc))) {
        ...
        nav->pclk = NULL;
        nav->nc = nav->ncmax = 0;
        trace(NULL, 1, "combpclk malloc error nc=%d\n", nav->nc);
        return;
    }
    nav->pclk = nav_pclk;
    nav->ncmax = nav->nc;

    trace(NULL, 4, "combpclk: nc=%d\n", nav->nc);
}
...
int open_rnxctr(rnxctr_t* rnx, FILE* fp) {
    ...
    int i, j, ver, sys, tsys;

    trace(NULL, 3, "open_rnxctr:\n");

    /* read RINEX header from file */
    if (!readrnxh(fp, &ver, &type, &sys, &tsys, tobs, &rnx->nav, &rnx->sta)) {
        trace(NULL, 2, "open_rnxctr: rinex header read error\n");
        return 0;
    }
    if (!strchr(rnxtypes, type)) {
        trace(NULL, 2, "open_rnxctr: not supported rinex type type=%c\n", type);
        return 0;
    }
    rnx->ver = ver;
    rnx->type = type;
...
int input_rnxctr(rnxctr_t* rnx, FILE* fp) {
    ...
    trace(NULL, 4, "input_rnxctr:\n");

    /* read RINEX OBS data */
    if (rnx->type == 'O') {
        if ((n = readrnxobsb(fp, rnx->opt, rnx->ver, &rnx->tsys, rnx->tobs, &flag, rnx->obs.data, &rnx->sta)) <= 0) {
            rnx->obs.n = 0;
            return n < 0 ? -2 : 0;
        }
...
static void outrnx_glo_bias(FILE* fp, const rnxopt_t* opt) {
    ...
    if (opt->navsys & SYS_GLO) {
        fprintf(fp, " %s %8.3f %s %8.3f %s %8.3f %s %8.3f%8s%-20s\n", tobs[0], opt->glo_cp_bias[0], tobs[1],
                opt->glo_cp_bias[1], tobs[2], opt->glo_cp_bias[2], tobs[3], opt->glo_cp_bias[3], "", label);
    } else {
        fprintf(fp, "%*s%-20s\n", 60, "", label);
    }
}
...
int outrnxobsh(FILE* fp, const rnxopt_t* opt, const nav_t* nav) {
    ...
    fprintf(fp, "%9.2f%-11s%-20s%-20s%-20s\n", opt->rnxver / 100.0, "", "OBSERVATION DATA", sys,
            "RINEX VERSION / TYPE");
    ...
    fprintf(fp, "  %04.0f    %02.0f    %02.0f    %02.0f    %02.0f   %010.7f     %-12s%-20s\n", ep[0], ep[1], ep[2],
            ep[3], ep[4], ep[5], tsys, "TIME OF FIRST OBS");

    time2epoch(opt->tend, ep);
    fprintf(fp, "  %04.0f    %02.0f    %02.0f    %02.0f    %02.0f   %010.7f     %-12s%-20s\n", ep[0], ep[1], ep[2],
            ep[3], ep[4], ep[5], tsys, "TIME OF LAST OBS");
...
static int obsindex(int rnxver, int sys, const uint8_t* code, const char* tobs, const char* mask) {
    ...
    for (i = 0; i < NFREQ + NEXOBS; i++) {
        ...
        if (rnxver <= 299) { /* ver.2 */
            if (!strcmp(tobs, "C1") &&
                (sys == SYS_GPS || sys == SYS_GLO || sys == SYS_QZS || sys == SYS_SBS || sys == SYS_CMP)) {
                if (code[i] == CODE_L1C) {
                    return i;
                }
...
int outrnxobsb(FILE* fp, const rnxopt_t* opt, const obsd_t* obs, int n, int flag) {
    ...
    if (opt->rnxver <= 299) { /* ver.2 */
        fprintf(fp, " %02d %02.0f %02.0f %02.0f %02.0f %010.7f  %d%3d", (int)ep[0] % 100, ep[1], ep[2], ep[3], ep[4],
                ep[5], flag, ns);
        for (i = 0; i < ns; i++) {
            if (i > 0 && i % 12 == 0) {
                fprintf(fp, "\n%32s", "");
    ...
    for (i = 0; i < ns; i++) {
        ...
        for (j = 0; j < opt->nobs[m]; j++) {
            ...
            if ((k = obsindex(opt->rnxver, sys, obs[ind[i]].code, opt->tobs[m][j], mask)) < 0) {
                outrnxobsf(fp, 0.0, -1);
                continue;
            }
            /* phase shift (cyc) */
            ...
            switch (opt->tobs[m][j][0]) {
                ...
                case 'P':
                    ...
                    break;
                case 'L':
                    outrnxobsf(fp, obs[ind[i]].L[k] + dL, obs[ind[i]].LLI[k]);
                    break;
                case 'D':
                    outrnxobsf(fp, obs[ind[i]].D[k], -1);
                    break;
                case 'S':
                    outrnxobsf(fp, obs[ind[i]].SNR[k] * SNR_UNIT, -1);
                    break;
            }
        }
...
static void outnavf_n(FILE* fp, double value, int n) {
    ...
    fprintf(fp, " %s.%0*.0f%s%+03.0f", value < 0.0 ? "-" : " ", n, fabs(value) / pow(10.0, e - n), NAVEXP, e);
}
static void outnavf(FILE* fp, double value) { outnavf_n(fp, value, 12); }
...
int outrnxnavh(FILE* fp, const rnxopt_t* opt, const nav_t* nav) {
    ...
    trace(NULL, 3, "outrnxnavh:\n");

    timestr_rnx(date);

    if (opt->rnxver <= 299) { /* ver.2 */
        fprintf(fp, "%9.2f           %-20s%-20s%-20s\n", opt->rnxver / 100.0, "N: GPS NAV DATA", "",
                "RINEX VERSION / TYPE");
    } else { /* ver.3 */
        if (opt->navsys == SYS_GPS) {
            sys = "G: GPS";
        ...
        fprintf(fp, "%9.2f           %-20s%-20s%-20s\n", opt->rnxver / 100.0, "N: GNSS NAV DATA", sys,
                "RINEX VERSION / TYPE");
    }
    fprintf(fp, "%-20.20s%-20.20s%-20.20s%-20s\n", opt->prog, opt->runby, date, "PGM / RUN BY / DATE");
...
int outrnxnavb(FILE* fp, const rnxopt_t* opt, const eph_t* eph) {
    ...
    if ((opt->rnxver >= 300 && sys == SYS_GPS) || (opt->rnxver >= 212 && sys == SYS_GAL) ||
        ...
        fprintf(fp, "%-3s %04.0f %02.0f %02.0f %02.0f %02.0f %02.0f", code, ep[0], ep[1], ep[2], ep[3], ep[4], ep[5]);
        sep = "    ";
    } else if (opt->rnxver <= 299 && sys == SYS_GPS) {
        fprintf(fp, "%2d %02d %02.0f %02.0f %02.0f %02.0f %04.1f", prn, (int)ep[0] % 100, ep[1], ep[2], ep[3], ep[4],
                ep[5]);
        sep = "   ";
    } else {
        return 0;
...
int outrnxgnavh(FILE* fp, const rnxopt_t* opt, const nav_t* nav) {
    ...
    trace(NULL, 3, "outrnxgnavh:\n");

    timestr_rnx(date);

    if (opt->rnxver <= 299) { /* ver.2 */
        fprintf(fp, "%9.2f           %-20s%-20s%-20s\n", opt->rnxver / 100.0, "GLONASS NAV DATA", "",
                "RINEX VERSION / TYPE");
    } else { /* ver.3 */
        fprintf(fp, "%9.2f           %-20s%-20s%-20s\n", opt->rnxver / 100.0, "N: GNSS NAV DATA", "R: GLONASS",
                "RINEX VERSION / TYPE");
    }
    fprintf(fp, "%-20.20s%-20.20s%-20.20s%-20s\n", opt->prog, opt->runby, date, "PGM / RUN BY / DATE");
...
int outrnxhnavh(FILE* fp, const rnxopt_t* opt, const nav_t* nav) {
    ...
    trace(NULL, 3, "outrnxhnavh:\n");

    timestr_rnx(date);

    if (opt->rnxver <= 299) { /* ver.2 */
        fprintf(fp, "%9.2f           %-20s%-20s%-20s\n", opt->rnxver / 100.0, "H: GEO NAV MSG DATA", "",
                "RINEX VERSION / TYPE");
    } else { /* ver.3 */
        fprintf(fp, "%9.2f           %-20s%-20s%-20s\n", opt->rnxver / 100.0, "N: GNSS NAV DATA", "S: SBAS Payload",
                "RINEX VERSION / TYPE");
    }
    fprintf(fp, "%-20.20s%-20.20s%-20.20s%-20s\n", opt->prog, opt->runby, date, "PGM / RUN BY / DATE");
...
int outrnxhnavb(FILE* fp, const rnxopt_t* opt, const seph_t* seph) {
    ...
    if (opt->rnxver <= 299) { /* ver.2 */
        fprintf(fp, "%2d %02d %2.0f %2.0f %2.0f %2.0f %4.1f", prn - 100, (int)ep[0] % 100, ep[1], ep[2], ep[3], ep[4],
                ep[5]);
        sep = "   ";
    } else { /* ver.3 */
...
int outrnxlnavh(FILE* fp, const rnxopt_t* opt, const nav_t* nav) {
    ...
    fprintf(fp, "%9.2f           %-20s%-20s%-20s\n", opt->rnxver / 100.0, "N: GNSS NAV DATA", "E: Galileo",
            "RINEX VERSION / TYPE");
...
int outrnxqnavh(FILE* fp, const rnxopt_t* opt, const nav_t* nav) {
    ...
    fprintf(fp, "%9.2f           %-20s%-20s%-20s\n", opt->rnxver / 100.0, "N: GNSS NAV DATA", "J: QZSS",
            "RINEX VERSION / TYPE");
...
int outrnxcnavh(FILE* fp, const rnxopt_t* opt, const nav_t* nav) {
    ...
    fprintf(fp, "%9.2f           %-20s%-20s%-20s\n", opt->rnxver / 100.0, "N: GNSS NAV DATA", "C: BeiDou",
            "RINEX VERSION / TYPE");
...
int outrnxinavh(FILE* fp, const rnxopt_t* opt, const nav_t* nav) {
    ...
    fprintf(fp, "%9.2f           %-20s%-20s%-20s\n", opt->rnxver / 100.0, "N: GNSS NAV DATA", "I: IRNSS",
            "RINEX VERSION / TYPE");
