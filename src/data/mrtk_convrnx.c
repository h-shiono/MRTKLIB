...
static void convcode(int rnxver, int sys, char* type) {
    if (rnxver >= 212 && (sys == SYS_GPS || sys == SYS_QZS || sys == SYS_SBS) && !strcmp(type + 1, "1C")) { /* L1C/A */
        strcpy(type + 1, "A");
    } else if (rnxver >= 212 && (sys == SYS_GPS || sys == SYS_QZS) &&
               (!strcmp(type + 1, "1S") || !strcmp(type + 1, "1L") || !strcmp(type + 1, "1X"))) { /* L1C */
        strcpy(type + 1, "B");
    } else if (rnxver >= 212 && (sys == SYS_GPS || sys == SYS_QZS) &&
               (!strcmp(type + 1, "2S") || !strcmp(type + 1, "2L") || !strcmp(type + 1, "2X"))) { /* L2C */
        strcpy(type + 1, "C");
        ...
        strcpy(type + 1, "D");
               ...
               (!strcmp(type + 1, "2I") || !strcmp(type + 1, "2Q") || !strcmp(type + 1, "2X"))) { /* B1_2 */
        strcpy(type + 1, "2");
    } else if (!strcmp(type, "C1P") || !strcmp(type, "C1W") || !strcmp(type, "C1Y") ||
               !strcmp(type, "C1N")) { /* L1P,P(Y) */
        strcpy(type, "P1");
    } else if (!strcmp(type, "C2P") || !strcmp(type, "C2W") || !strcmp(type, "C2Y") || !strcmp(type, "C2N") ||
...
static int input_strfile(strfile_t* str) {
    ...
    if (str->format == STRFMT_RTCM2) {
        ...
        if ((type = input_rawf(&str->raw, &str->rtcm, str->format, str->fp)) >= 1) {
...
static void setopt_obstype(const uint8_t* codes, const uint8_t* types, int sys, rnxopt_t* opt) {
    ...
    trace(NULL, 3, "setopt_obstype: sys=%d\n", sys);
    ...
    for (i = 0; codes[i]; i++) {
        ...
        if (opt->rnxver >= 300) {
            ...
            if (ver < '0' || ver > '0' + opt->rnxver - 300) {
                trace(NULL, 2, "unsupported obs type: rnxver=%.2f sys=%d code=%s\n", opt->rnxver / 100.0, sys,
                      code2obs(codes[i]));
                continue;
            }
        }
...
static void setopt_phshift(rnxopt_t* opt) {
    ...
    for (i = 0; i < NSATSYS; i++)
        for (j = 0; j < opt->nobs[i]; j++) {
            ...
            if (navsys[i] == SYS_GPS) {
                if (code == CODE_L1S || code == CODE_L1L || code == CODE_L1X || code == CODE_L1P || code == CODE_L1W ||
                    code == CODE_L1N) {
...
static void setopt_sta_list(const strfile_t* str, rnxopt_t* opt) {
    ...
    sprintf(opt->comment[i++], "%5s  %22s  %22s", "STAID", "TIME OF FIRST OBS", "TIME OF LAST OBS");
...
static void dump_stas(const strfile_t* str) {
    ...
    trace(NULL, 2, "# STATION LIST\n");
    trace(NULL, 2, "# %17s %19s %5s %6s %16s %16s %12s %13s %9s %2s %6s %6s %6s\n", "TIME", "STAID", "MARKER",
          "ANTENNA", "RECEIVER", "LATITUDE", "LONGITUDE", "HIGHT", "DT", "DEL1", "DEL2", "DEL3");
    ...
    for (p = str->stas; p; p = p->next) {
        ...
        trace(NULL, 2,
              ...
              "%6.3f %6.3f %6.3f\n",
              s1, s2, p->staid, p->sta.name, p->sta.antdes, p->sta.rectype, pos[0] * R2D, pos[1] * R2D, pos[2],
              p->sta.deltype, p->sta.del[0], p->sta.del[1], p->sta.del[2]);
    }
#endif
}
...
static void dump_halfc(const strfile_t* str) {
    ...
    trace(NULL,2,"# HALF-CYCLE AMBIGUITY CORRECTIONS\n");
    trace(NULL,2,"# %20s %22s %4s %3s %3s\n","START","END","SAT","FRQ","COR");
    ...
    for (i = 0; i < n; i++)
        for (j = 0; j < NFREQ + NEXOBS; j++) {
            ...
            for (p = str->halfc[sat - 1][j]; p; p = p->next) {
                if (p->stat <= 1) continue;
                if (timediff(data[i].time, p->ts) < -DTTOL || timediff(data[i].time, p->te) > DTTOL) continue;
...
/* scan input files ----------------------------------------------------------*/
static int scan_file(char** files, int nf, rnxopt_t* opt, strfile_t* str, int* mask) {
    eph_t eph0 = {0, -1, -1};
    geph_t geph0 = {0, -1};
    seph_t seph0 = {0};
    ...
    for (m = 0; m < nf && !abort; m++) {
        ...
        while ((type = input_strfile(str)) >= -1) {
            ...
            sprintf(msg, "scanning: %s %s%s%s%s%s%s%s", time_str(str->time, 0), n[0] ? "G" : "", n[1] ? "R" : "",
                    n[2] ? "E" : "", n[3] ? "J" : "", n[4] ? "S" : "", n[5] ? "C" : "", n[6] ? "I" : "");
    ...
    if (abort) {
        trace(NULL, 2, "aborted in scan\n");
        return 0;
    }
    for (i = 0; i < NSATSYS; i++)
        for (j = 0; j < n[i]; j++) {
            trace(NULL, 2, "scan_file: sys=%d code=%s type=%d\n", i, code2obs(codes[i][j]), types[i][j]);
        }
    ...
    for (i = 0; i < NSATSYS; i++) {
        ...
        for (j = 0; j < n[i]; j++) {
            trace(NULL, 3, "scan_file: sys=%d code=%s\n", i, code2obs(codes[i][j]));
        }
    }
    /* set station info in RINEX options */
    setopt_sta(str, opt);
...
/* open output files ---------------------------------------------------------*/
static int openfile(FILE** ofp, char* files[], const char* file, const rnxopt_t* opt, const nav_t* nav) {
    char path[1024];
    int i;

    trace(NULL, 3, "openfile:\n");
...
/* output RINEX event --------------------------------------------------------*/
static void outrnxevent(FILE* fp, const rnxopt_t* opt, gtime_t time, int event, const stas_t* stas, int staid) {
    const stas_t *p = NULL, *q;
    double ep[6], pos[3], enu[3], del[3];

    trace(NULL, 3, "outrnxevent: event=%d\n", event);
    ...
    if (event == EVENT_STARTMOVE) {
        ...
        fprintf(fp, "%-20.20s%-20.20s%-20.20s%-20s\n", p->sta.recsno, p->sta.rectype, p->sta.recver,
                "REC # / TYPE / VERS");
        fprintf(fp, "%-20.20s%-20.20s%-20.20s%-20s\n", p->sta.antsno, p->sta.antdes, "", "ANT # / TYPE");
        ...
        fprintf(fp, "%s %02d %02.0f %02.0f %02.0f %02.0f %010.7f  %d%3d\n", (opt->rnxver >= 300) ? ">" : "",
                (opt->rnxver >= 300) ? (int)ep[0] : (int)ep[0] % 100, ep[1], ep[2], ep[3], ep[4], ep[5], event, 1);
...
static int screent_ttol(gtime_t time, gtime_t ts, gtime_t te, double tint, double ttol) {
    ...
    return (tint <= 0.0 || fmod(time2gpst(time, NULL) + ttol, tint) <= ttol * 2.0) &&
           (ts.time == 0 || timediff(time, ts) >= -ttol) && (te.time == 0 || timediff(time, te) < ttol);
}
...
static void convsbs(FILE** ofp, rnxopt_t* opt, strfile_t* str, int* n, gtime_t* tend) {
    gtime_t time;
    int prn, sat, sys, sep_nav = opt->rnxver <= 299 || opt->sep_nav;

    trace(NULL, 3, "convsbs :\n");
    ...
    *tend = time;
    ...
    if (MINPRNSBS <= prn && prn <= MAXPRNSBS) {
        ...
        trace(NULL, 2, "sbas message satellite error: prn=%d\n", prn);
        return;
    }
...
static void setopt_apppos(strfile_t* str, rnxopt_t* opt) {
    ...
    if (!pntpos(NULL, str->obs->data, str->obs->n, str->nav, &prcopt, &sol, NULL, NULL, msg)) {
        trace(NULL, 2, "point position error (%s)\n", msg);
        return;
    }
...
static int convrnx_s(int sess, int format, rnxopt_t* opt, const char* file, char** ofile) {
    ...
    trace(NULL, 3,
          ...
          "%s %s\n",
          sess, format, file, ofile[0], ofile[1], ofile[2], ofile[3], ofile[4], ofile[5], ofile[6], ofile[7], ofile[8]);
    ...
    for (i = 0; i < NOUTFILE; i++) {
        paths[i] = s[i];
        if (reppath(ofile[i], paths[i], opt->ts.time ? opt->ts : str->tstart, staname, "") < 0) {
...
extern int convrnx(int format, rnxopt_t* opt, const char* file, char** ofile) {
    ...
    trace(NULL, 3, "convrnx: format=%d file=%s ofile=%s %s %s %s %s %s %s %s %s\n", format, file, ofile[0], ofile[1],
          ofile[2], ofile[3], ofile[4], ofile[5], ofile[6], ofile[7], ofile[8]);
