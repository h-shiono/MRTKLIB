...
static int decode_stat(sstat_t* sstat, char* buff, int* nbyte) {
    ...
    *nbyte = 0;
    /* point position */
    if (sscanf(buff, "$POS,%d,%lf,%d,%lf,%lf,%lf", &week, &tow, &qflag, ecef, ecef + 1, ecef + 2) >= 6) {
        sstat->time = gpst2time(week, tow);
        memcpy(sstat->sion.site.ecef, ecef, sizeof(ecef));
        memcpy(sstat->strp.site.ecef, ecef, sizeof(ecef));
    }
    /* ionosphere */
    else if (sscanf(buff, "$ION,%d,%lf,%d,%c%d,%lf,%lf,%lf,%lf", &week, &tow, &qflag, &sys, &prn, azel, azel + 1, &ion,
                    std) >= 9) {
    ...
    /* troposphere ztd */
    else if (sscanf(buff, "$TROP,%d,%lf,%d,%d,%lf,%lf", &week, &tow, &qflag, &rcv, &ztd, std) >= 6) {
        sstat->time = gpst2time(week, tow);
    ...
    else if (sscanf(buff, "$TRPG,%d,%lf,%d,%d,%lf,%lf,%lf,%lf", &week, &tow, &qflag, &rcv, grad, grad + 1, std,
                    std + 1) >= 8) {
...
/* get neighbour sites -------------------------------------------------------*/
static int get_site(const stat_t* stat, const double* llh, int iontrp, int* siteno, double maxdist, double* dist,
                    double gllh[][3]) {
    double ecef[3], d;
    int i, n = 0, nsta;
    const spos_t* site;
    ...
    for (i = 0; i < nsta; i++) {
        ...
        trace(NULL, 4, "get_site:targetpos llh=%f,%f,%f site pos llh=%f,%f,%f dist=%f\n", llh[0] * R2D, llh[1] * R2D,
              llh[2], gllh[n][0] * R2D, gllh[n][1] * R2D, gllh[n][2], d);
...
extern int get_trop_sta(gtime_t time, const trp_t* trpd, double* trp, double* std) {
    double tt;
    int i;
...
extern int get_iono_sta(gtime_t time, const ion_t* iond, double* ion, double* std, double* el) {
    double tt;
    int i, cnt = 0;
 ...
 *-----------------------------------------------------------------------------*/
extern int corr_trop_distwgt(const stat_t* stat, gtime_t time, const double* llh, double maxdist, double* trp,
                             double* std) {
    ...
    for (i = 0; i < 3; i++) {
        ...
        trace(NULL, 3, "interpolation trp  %s %d trp=%f std=%f lat=%.2f lat=%.2f n=%d\n", time_str(time, 0), i + 1,
              trp[i], std[i], llh[0] * R2D, llh[1] * R2D, n);
    }
    /* convert zwd to ztd */
 ...
 * return : status (1:ok, 0:error)
 *-----------------------------------------------------------------------------*/
extern int corr_iono_distwgt(const stat_t* stat, gtime_t time, const double* llh, const double* el, double maxdist,
    ...
    for (i = 0; i < n; i++) {
        ...
        if (get_iono_sta(time, stat->sion[siteno[i]].iond, ion_s[i], std_s, el_s) == 0) {
            continue;
        }
    ...
    for (i = 0; i < MAXSAT; i++) {
        ...
        trace(NULL, 3, "interpolation ion  %s sat=%d ion=%f std=%f lat=%.2f lat=%.2f n=%d\n", time_str(time, 0), i + 1,
              ion[i], std[i], llh[0] * R2D, llh[1] * R2D, n);
    }
    if (maxres > 0.0) {
        for (i = 0; i < n; i++) {
            for (j = 0; j < MAXSAT; j++) {
