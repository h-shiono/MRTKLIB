...
static int plane(const double p1[3], const double p2[3], const double p3[3], double peq[4]) {
    double a2b[3], a2c[3];
...
static int intpt(const double p1[3], const double p2[3], const double p3[3], double t[3]) {
    double peq[4];
...
static int selpersta(const stat_t* stat, const double* llh, int flag, double maxdist, int* id) {
    int i, j, k, n = 0;
    double d;
    ...
    for (i = 0; i < (flag == TYPE_TRP ? stat->nst : stat->nsi); i++) {
        ...
        trace(NULL, 4, "get_site:targetpos llh=%f,%f,%f site pos llh=%f,%f,%f dist=%f\n", tllh[0][0] * R2D,
              tllh[0][1] * R2D, tllh[0][2], tllh[1][0] * R2D, tllh[1][1] * R2D, tllh[1][2], d);
        if (d > maxdist * 1E3) {
            continue;
        }
...
/* collision detection -------------------------------------------------------*/
static int coldet(const double p1[3], const double p2[3], const double p3[3], const double tgt[3]) {
    double z[3] = {0.0};
...
static int gettriangledd(const stat_t* stat, const double* llh, int flag, const double maxdist, const int extp,
                         int* staid) {
    ...
    for (i = 0; i < stanum && i < MAXSITES; i++) {
        ...
        for (j = i + 1; j < stanum && j < MAXSITES; j++) {
            ...
            for (k = j + 1; k < stanum && k < MAXSITES; k++) {
                ...
                if (ret) {
                    trace(NULL, 3, "interpolation: %.1f %.1f site=%s %s %s\n", llh[0] * R2D, llh[1] * R2D, site[0].name,
                          site[1].name, site[2].name);
                    return 3;
                }
            }
        }
...
static int corr_trop_plane(const stat_t* stat, gtime_t time, const double* llh, double maxdist, int extp, double* trp,
                           double* std) {
    int i, j, staid[3];
...
static int corr_iono_plane(const stat_t* stat, gtime_t time, const double* llh, double maxdist, int extp, double* ion,
    ...
    /* convert station position & time interpolation */
    for (i = 0; i < 3; i++) {
        if (get_iono_sta(time, stat->sion[siteno[i]].iond, ion_s[i], std_s[i], el_s[i]) == 0) {
            return 0;
        }
 ...
 *-----------------------------------------------------------------------------*/
extern void grid_intp_trop(const gtime_t gt, const stat_t* stat, lclblock_t* lclblk) {
    int i, j, n = 0;
    blkinf_t* bi;
    trp_t* trpp;
    ...
    for (i = 0; i < lclblk->tnum; i++) {
        ...
        for (j = 0, n = 0; j < GN(bi->gpitch); j++) {
            ...
            if (lclblk->interp == MODE_PLANE) {
                if (!corr_trop_plane(stat, gt, bi->grid[j], lclblk->tdist, lclblk->extp, trpp->trp, trpp->std)) {
                    continue;
                }
            } else {
                if (!corr_trop_distwgt(stat, gt, bi->grid[j], lclblk->tdist, trpp->trp, trpp->std)) {
                    continue;
                }
            }
            trpp->time = gt;
 ...
 *-----------------------------------------------------------------------------*/
extern void sta_sel_trop(const gtime_t time, const stat_t* stat, lclblock_t* lslblk) {
    int i, j, k;
    double llh[3];
    blkinf_t* bi;
    sitetrp_t* strp;
 ...
 *-----------------------------------------------------------------------------*/
extern void grid_intp_iono(const gtime_t gt, const stat_t* stat, lclblock_t* lclblk, double maxres, int* nrej,
                           int* ncnt) {
    int i, j, k, n = 0;
    ...
    for (i = 0; i < lclblk->inum; i++) {
        ...
        for (j = 0, n = 0; j < GN(bi->gpitch); j++) {
            if (lclblk->interp == MODE_PLANE) {
                if (!corr_iono_plane(stat, gt, bi->grid[j], lclblk->idist, lclblk->extp, ion, std, maxres, nrej,
                                     ncnt)) {
                    continue;
                }
            } else {
                if (!corr_iono_distwgt(stat, gt, bi->grid[j], NULL, lclblk->idist, ion, std, maxres, nrej, ncnt)) {
                    continue;
                }
            }
            for (k = 0; k < MAXSAT; k++) {
 ...
 *          lclblock_t *lslblk IO   local block information struct
 *-----------------------------------------------------------------------------*/
extern void sta_sel_iono(const gtime_t gt, const stat_t* stat, lclblock_t* lslblk) {
    int i, j, k;
    double llh[3];
    blkinf_t* bi;
    siteion_t* sion;
    ...
    for (i = 0; i < lslblk->tnum; i++) {
        ...
        for (j = 0; j < stat->nsi; j++) {
            ...
            for (k = 0; k < MAXSAT; k++) {
                ...
                if (tt > 0.0) {
                    memcpy(&sion->iond[k], &stat->sion[j].iond[k], sizeof(sion->iond[k]));
                } else {
                    sion->iond[k].ion = 0.0;
                    sion->iond[k].std = 0.0;
                }
