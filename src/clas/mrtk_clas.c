...
static clas_bias_bank_t* get_close_cbias(clas_bank_ctrl_t* bank, gtime_t time, int network, double age) {
    ...
    for (i = 0; i < CLAS_BANK_NUM; i++) {
        ...
        if (timediff(bank->BiasBank[i].time, time) > age) {
            continue;
        }
        if (pos != -1 && timediff(bank->BiasBank[pos].time, bank->BiasBank[i].time) > 0.0) {
            continue;
        }
        if ((bank->BiasBank[i].bflag & 0x01) == 0x01) {
            pos = i;
        }
    }
...
static clas_bias_bank_t* get_close_pbias(clas_bank_ctrl_t* bank, gtime_t time, int network, double age) {
    ...
    for (i = 0; i < CLAS_BANK_NUM; i++) {
        ...
        if ((bank->BiasBank[i].bflag & 0x02) == 0x02) {
            pos = i;
        }
    }
    return (pos != -1) ? &bank->BiasBank[pos] : NULL;
...
static clas_trop_bank_t* get_close_trop(clas_bank_ctrl_t* bank, gtime_t obstime, gtime_t time, int network, double age,
    ...
    for (i = 0; i < CLAS_BANK_NUM; i++) {
        if (!bank->TropBank[i].use) {
            continue;
        }
        if (fabs(timediff(time, bank->TropBank[i].time)) > age) {
            continue;
        }
        if (pos != -1 && timediff(bank->TropBank[pos].time, bank->TropBank[i].time) > 0.0) {
            continue;
        }
        if (tropless && timediff(obstime, bank->TropBank[i].time) < 0.0) {
            continue;
        }
        if (bank->TropBank[i].gridnum[network - 1] > 0) {
            pos = i;
        }
    }
...
static void set_cssr_bank_orbit(clas_ctx_t* ctx, int ch, gtime_t time, int network) {
    ...
    if ((orbit = get_same_orbit(bank, time, network)) == NULL) {
        ...
        orbit->time = time;
        orbit->use = 1;
        if (++bank->NextOrbit >= CLAS_BANK_NUM) {
            bank->NextOrbit = 0;
        }
...
static void set_cssr_bank_clock(clas_ctx_t* ctx, int ch, gtime_t time, int network) {
    ...
    if ((clk = get_same_clock(bank, time, network)) == NULL) {
        ...
        clk->time = time;
        clk->use = 1;
        if (++bank->NextClock >= CLAS_BANK_NUM) {
            bank->NextClock = 0;
        }
...
static void set_cssr_bank_cbias(clas_ctx_t* ctx, int ch, gtime_t time, int network, int iod) {
    ...
    if ((bias = get_same_bias(bank, time, network)) == NULL) {
        ...
        bias->time = time;
        bias->use = 1;
        if (++bank->NextBias >= CLAS_BANK_NUM) {
            bank->NextBias = 0;
        }
...
static void set_cssr_bank_trop(clas_ctx_t* ctx, int ch, gtime_t time, ssrn_t* ssrn, int network) {
    ...
    if ((trop = get_same_trop(bank, time)) == NULL) {
        ...
        trop->time = time;
        trop->use = 1;
        if (++bank->NextTrop >= CLAS_BANK_NUM) {
            bank->NextTrop = 0;
        }
...
static int decode_cssr_combo(clas_ctx_t* ctx, int ch, clas_l6buf_t* l6, int i0) {
    ...
    if (flg_orbit) {
        set_cssr_bank_orbit(ctx, ch, l6->time, flg_net ? netid : 0);
    }
    if (flg_clock) {
        set_cssr_bank_clock(ctx, ch, l6->time, flg_net ? netid : 0);
    }
...
static int decode_cssr_atmos(clas_ctx_t* ctx, int ch, clas_l6buf_t* l6, int i0) {
    ...
    for (j = 0; j < ssrn->ngp && j < CLAS_MAX_GP; j++) {
        ...
        if (trop_type > 0) {
            ssrn->trop_total[j] += ct[1] * dlat + ct[2] * dlon;
        }
        if (trop_type > 1) {
            ssrn->trop_total[j] += ct[3] * dlat * dlon;
        }
    }
...
static int decode_qzs_msg(clas_ctx_t* ctx, int ch) {
    ...
    switch (l6->subtype) {
        case CSSR_TYPE_MASK:
            if (!check_bit_width_mask(l6, cssr, i)) {
                return 0;
            }
            break;
        case CSSR_TYPE_OC:
            if (!check_bit_width_oc(l6, cssr, i)) {
                return 0;
            }
            break;
        case CSSR_TYPE_CC:
            if (!check_bit_width_cc(l6, cssr, i)) {
                return 0;
            }
            break;
        case CSSR_TYPE_CB:
            if (!check_bit_width_cb(l6, cssr, i)) {
                return 0;
            }
            break;
        case CSSR_TYPE_PB:
            if (!check_bit_width_pb(l6, cssr, i)) {
                return 0;
            }
            break;
        case CSSR_TYPE_BIAS:
            if (!check_bit_width_bias(l6, cssr, i)) {
                return 0;
            }
            break;
        case CSSR_TYPE_URA:
            if (!check_bit_width_ura(l6, cssr, i)) {
                return 0;
            }
            break;
        case CSSR_TYPE_STEC:
            if (!check_bit_width_stec(l6, cssr, i)) {
                return 0;
            }
            break;
        case CSSR_TYPE_GRID:
            if (!check_bit_width_grid(l6, cssr, i)) {
                return 0;
            }
            break;
        case CSSR_TYPE_COMBO:
            if (!check_bit_width_combo(l6, cssr, i)) {
                return 0;
            }
            break;
        case CSSR_TYPE_ATMOS:
            if (!check_bit_width_atmos(l6, cssr, i)) {
                return 0;
            }
            break;
        case CSSR_TYPE_SI:
            if (!check_bit_width_si(l6, cssr, i)) {
                return 0;
            }
            break;
        case 0:
...
extern int clas_input_cssrf(clas_ctx_t* ctx, FILE* fp, int ch) {
    ...
    for (i = 0; i < 4096; i++) {
        if ((ret = decode_qzs_msg(ctx, ch))) {
            return ret;
        }
        if ((data = fgetc(fp)) == EOF) {
            return -2;
        }
        if ((ret = clas_input_cssr(ctx, (uint8_t)data, ch))) {
            return ret;
        }
    }
    return 0;
