...
static int encode_type1020(rtcm_t *rtcm, int sync)
{
    ...
    setbitu(rtcm->buff,i, 4,geph->sva); i+= 4; /* FT */
    setbitu(rtcm->buff,i,11,NT       ); i+=11;
    setbitu(rtcm->buff,i, 2,M        ); i+= 2; /* M */
    ...
    setbitu(rtcm->buff,i, 1,0        ); i+= 1; /* ln (5th string) */
                           ...
                           int provid, int solid)
{
    ...
    trace(NULL,4,"encode_ssr_head: type=%d sys=%d subtype=%d nsat=%d sync=%d iod=%d "
