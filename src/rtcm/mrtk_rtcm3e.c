...
static void gen_obs_gps(rtcm_t* rtcm, const obsd_t* data, int* code1, int* pr1, int* ppr1, int* lock1, int* amb,
                        int* cnr1, int* code2, int* pr21, int* ppr2, int* lock2, int* cnr2) {
    ...
    *pr1 = *amb = 0;
    ...
    /* L2 -L1 pseudorange */
    if (data->P[0] != 0.0 && data->P[1] != 0.0 && data->code[0] && data->code[1] && fabs(data->P[1] - pr1c) <= 163.82) {
        if (pr21) {
...
static void gen_obs_glo(rtcm_t* rtcm, const obsd_t* data, int fcn, int* code1, int* pr1, int* ppr1, int* lock1,
                        int* amb, int* cnr1, int* code2, int* pr21, int* ppr2, int* lock2, int* cnr2) {
...
static int encode_head(int type, rtcm_t* rtcm, int sys, int sync, int nsat) {
    ...
    trace(NULL, 4, "encode_head: type=%d sync=%d sys=%d nsat=%d\n", type, sync, sys, nsat);
...
static int encode_type1001(rtcm_t* rtcm, int sync) {
    ...
    trace(NULL, 3, "encode_type1001: sync=%d\n", sync);
    ...
    for (j = 0; j < rtcm->obs.n && nsat < MAXOBS; j++) {
        ...
        /* generate obs field data gps */
        gen_obs_gps(rtcm, rtcm->obs.data + j, &code1, &pr1, &ppr1, &lock1, &amb, NULL, NULL, NULL, NULL, NULL, NULL);
...
static int encode_type1002(rtcm_t* rtcm, int sync) {
    ...
    trace(NULL, 3, "encode_type1002: sync=%d\n", sync);
    ...
    for (j = 0; j < rtcm->obs.n && nsat < MAXOBS; j++) {
        ...
        /* generate obs field data gps */
        gen_obs_gps(rtcm, rtcm->obs.data + j, &code1, &pr1, &ppr1, &lock1, &amb, &cnr1, NULL, NULL, NULL, NULL, NULL);
...
static int encode_type1003(rtcm_t* rtcm, int sync) {
    ...
    trace(NULL, 3, "encode_type1003: sync=%d\n", sync);
    ...
    for (j = 0; j < rtcm->obs.n && nsat < MAXOBS; j++) {
        ...
        /* generate obs field data gps */
        gen_obs_gps(rtcm, rtcm->obs.data + j, &code1, &pr1, &ppr1, &lock1, &amb, NULL, &code2, &pr21, &ppr2, &lock2,
                    NULL);
...
static int encode_type1004(rtcm_t* rtcm, int sync) {
    ...
    trace(NULL, 3, "encode_type1004: sync=%d\n", sync);
    ...
    for (j = 0; j < rtcm->obs.n && nsat < MAXOBS; j++) {
        ...
        /* generate obs field data gps */
        gen_obs_gps(rtcm, rtcm->obs.data + j, &code1, &pr1, &ppr1, &lock1, &amb, &cnr1, &code2, &pr21, &ppr2, &lock2,
                    &cnr2);
...
static int encode_type1005(rtcm_t* rtcm, int sync) {
    ...
    int i = 24;
    int j, has_gps = 0, has_glo = 0, has_gal = 0;

    trace(NULL, 3, "encode_type1005: sync=%d\n", sync);
...
static int encode_type1007(rtcm_t* rtcm, int sync) {
    ...
    trace(NULL, 3, "encode_type1007: sync=%d\n", sync);
...
static int encode_type1008(rtcm_t* rtcm, int sync) {
    ...
    trace(NULL, 3, "encode_type1008: sync=%d\n", sync);
...
static int encode_type1009(rtcm_t* rtcm, int sync) {
    ...
    for (j = 0; j < rtcm->obs.n && nsat < MAXOBS; j++) {
        ...
        /* generate obs field data glonass */
        gen_obs_glo(rtcm, rtcm->obs.data + j, fcn, &code1, &pr1, &ppr1, &lock1, &amb, NULL, NULL, NULL, NULL, NULL,
                    NULL);
...
static int encode_type1010(rtcm_t* rtcm, int sync) {
    ...
    trace(NULL, 3, "encode_type1010: sync=%d\n", sync);
    ...
    for (j = 0; j < rtcm->obs.n && nsat < MAXOBS; j++) {
        ...
        /* generate obs field data glonass */
        gen_obs_glo(rtcm, rtcm->obs.data + j, fcn, &code1, &pr1, &ppr1, &lock1, &amb, &cnr1, NULL, NULL, NULL, NULL,
                    NULL);
...
static int encode_type1011(rtcm_t* rtcm, int sync) {
    ...
    trace(NULL, 3, "encode_type1011: sync=%d\n", sync);
    ...
    for (j = 0; j < rtcm->obs.n && nsat < MAXOBS; j++) {
        ...
        /* generate obs field data glonass */
        gen_obs_glo(rtcm, rtcm->obs.data + j, fcn, &code1, &pr1, &ppr1, &lock1, &amb, NULL, &code2, &pr21, &ppr2,
                    &lock2, NULL);
...
static int encode_type1012(rtcm_t* rtcm, int sync) {
    ...
    trace(NULL, 3, "encode_type1012: sync=%d\n", sync);
    ...
    for (j = 0; j < rtcm->obs.n && nsat < MAXOBS; j++) {
        ...
        /* generate obs field data glonass */
        gen_obs_glo(rtcm, rtcm->obs.data + j, fcn, &code1, &pr1, &ppr1, &lock1, &amb, &cnr1, &code2, &pr21, &ppr2,
                    &lock2, &cnr2);
...
static int encode_type1019(rtcm_t* rtcm, int sync) {
    ...
    trace(NULL, 3, "encode_type1019: sync=%d\n", sync);
...
static int encode_type1020(rtcm_t* rtcm, int sync) {
    ...
    int fcn, NT, Bn, Cn, Cn_a, ln, P1, P2, P3, P4, P, M;

    trace(NULL, 3, "encode_type1020: sync=%d\n", sync);
    ...
    setbitu(rtcm->buff, i, 5, fcn);
    i += 5;
    setbitu(rtcm->buff, i, 1, Cn);
    i += 1;
    setbitu(rtcm->buff, i, 1, Cn_a);
    i += 1;
    setbitu(rtcm->buff, i, 2, P1);
    i += 2;
    ...
    i += 1;
    setbitu(rtcm->buff, i, 1, Bn);
    i += 1;
    setbitu(rtcm->buff, i, 1, P2);
    i += 1;
    ...
    i += 5;
    setbitu(rtcm->buff, i, 1, P3);
    i += 1;
    setbitg(rtcm->buff, i, 11, gamn);
    i += 11;
    setbitu(rtcm->buff, i, 2, P);
    i += 2;
    setbitu(rtcm->buff, i, 1, ln);
    i += 1;
    ...
    i += 5; /* En */
    setbitu(rtcm->buff, i, 1, P4);
    i += 1;
    setbitu(rtcm->buff, i, 4, geph->sva);
    i += 4; /* FT */
    setbitu(rtcm->buff, i, 11, NT);
    i += 11;
    setbitu(rtcm->buff, i, 2, M);
    i += 2; /* M */
    setbitu(rtcm->buff, i, 1, 0);
...
static int encode_type1033(rtcm_t* rtcm, int sync) {
    ...
    trace(NULL, 3, "encode_type1033: sync=%d\n", sync);
...
static int encode_type1041(rtcm_t* rtcm, int sync) {
    ...
    trace(NULL, 3, "encode_type1041: sync=%d\n", sync);
...
static int encode_type1044(rtcm_t* rtcm, int sync) {
    ...
    trace(NULL, 3, "encode_type1044: sync=%d\n", sync);
...
static int encode_type1045(rtcm_t* rtcm, int sync) {
    ...
    trace(NULL, 3, "encode_type1045: sync=%d\n", sync);
...
static int encode_type1046(rtcm_t* rtcm, int sync) {
    ...
    trace(NULL, 3, "encode_type1046: sync=%d\n", sync);
...
static int encode_type1042(rtcm_t* rtcm, int sync) {
    ...
    trace(NULL, 3, "encode_type1042: sync=%d\n", sync);
...
static int encode_type63(rtcm_t* rtcm, int sync) {
    ...
    trace(NULL, 3, "encode_type63: sync=%d\n", sync);
...
/* encode SSR header ---------------------------------------------------------*/
static int encode_ssr_head(int type, rtcm_t* rtcm, int sys, int subtype, int nsat, int sync, int iod, double udint,
                           int refd, int provid, int solid) {
    double tow;
...
static int encode_ssr1(rtcm_t* rtcm, int sys, int subtype, int sync) {
    ...
    trace(NULL, 3, "encode_ssr1: sys=%d subtype=%d sync=%d\n", sys, subtype, sync);
...
static int encode_ssr2(rtcm_t* rtcm, int sys, int subtype, int sync) {
    ...
    trace(NULL, 3, "encode_ssr2: sys=%d subtype=%d sync=%d\n", sys, subtype, sync);
...
static int encode_ssr3(rtcm_t* rtcm, int sys, int subtype, int sync) {
    ...
    trace(NULL, 3, "encode_ssr3: sys=%d subtype=%d sync=%d\n", sys, subtype, sync);
    ...
    for (j = nsat = 0; j < MAXSAT; j++) {
        ...
        for (k = nbias = 0; k < 32; k++) {
            if (!sigs[k] || rtcm->ssr[j].vcbias[sigs[k] - 1] == 0) {
                continue;
            }
            code[nbias] = k;
...
static int encode_ssr4(rtcm_t* rtcm, int sys, int subtype, int sync) {
    ...
    trace(NULL, 3, "encode_ssr4: sys=%d subtype=%d sync=%d\n", sys, subtype, sync);
...
static int encode_ssr5(rtcm_t* rtcm, int sys, int subtype, int sync) {
    ...
    trace(NULL, 3, "encode_ssr5: sys=%d subtype=%d sync=%d\n", sys, subtype, sync);
...
static int encode_ssr6(rtcm_t* rtcm, int sys, int subtype, int sync) {
    ...
    trace(NULL, 3, "encode_ssr6: sys=%d subtype=%d sync=%d\n", sys, subtype, sync);
...
static int encode_ssr7(rtcm_t* rtcm, int sys, int subtype, int sync) {
    ...
    for (j = nsat = 0; j < MAXSAT; j++) {
        ...
        for (k = nbias = 0; k < 32; k++) {
            if (!sigs[k] || rtcm->ssr[j].vpbias[sigs[k] - 1] == 0) {
                continue;
            }
            code[nbias] = k;
        ...
        for (k = 0; k < nbias; k++) {
            ...
            setbitu(rtcm->buff, i, 4, discnt[k]);
            i += 4; /* discont counter */
...
static void gen_msm_index(rtcm_t* rtcm, int sys, int* nsat, int* nsig, int* ncell, uint8_t* sat_ind, uint8_t* sig_ind,
                          uint8_t* cell_ind) {
...
static void gen_msm_sat(rtcm_t* rtcm, int sys, int nsat, const uint8_t* sat_ind, double* rrng, double* rrate,
                        uint8_t* info) {
    obsd_t* data;
    double freq;
...
static void gen_msm_sig(rtcm_t* rtcm, int sys, int nsat, int nsig, int ncell, const uint8_t* sat_ind,
                        const uint8_t* sig_ind, const uint8_t* cell_ind, const double* rrng, const double* rrate,
                        double* psrng, double* phrng, double* rate, double* lock, uint8_t* half, float* cnr) {
    obsd_t* data;
...
/* encode MSM header ---------------------------------------------------------*/
static int encode_msm_head(int type, rtcm_t* rtcm, int sys, int sync, int* nsat, int* ncell, double* rrng,
                           double* rrate, uint8_t* info, double* psrng, double* phrng, double* rate, double* lock,
                           uint8_t* half, float* cnr) {
    double tow;
    ...
    /* generate msm signal data fields */
    gen_msm_sig(rtcm, sys, *nsat, nsig, *ncell, sat_ind, sig_ind, cell_ind, rrng, rrate, psrng, phrng, rate, lock, half,
                cnr);
...
static int encode_msm_int_rrng(rtcm_t* rtcm, int i, const double* rrng, int nsat) {
    ...
    for (j = 0; j < nsat; j++) {
        if (rrng[j] == 0.0) {
            ...
            trace(NULL, 2, "msm rough range overflow %s rrng=%.3f\n", time_str(rtcm->time, 0), rrng[j]);
            int_ms = 255;
        } else {
...
static int encode_msm_rrate(rtcm_t* rtcm, int i, const double* rrate, int nsat) {
    ...
    for (j = 0; j < nsat; j++) {
        if (fabs(rrate[j]) > 8191.0) {
            trace(NULL, 2, "msm rough phase-range-rate overflow %s rrate=%.4f\n", time_str(rtcm->time, 0), rrate[j]);
            rrate_val = -8192;
        } else {
...
static int encode_msm_psrng(rtcm_t* rtcm, int i, const double* psrng, int ncell) {
    ...
    for (j = 0; j < ncell; j++) {
        if (psrng[j] == 0.0) {
            ...
            trace(NULL, 2, "msm fine pseudorange overflow %s psrng=%.3f\n", time_str(rtcm->time, 0), psrng[j]);
            psrng_val = -16384;
        } else {
...
static int encode_msm_psrng_ex(rtcm_t* rtcm, int i, const double* psrng, int ncell) {
    ...
    for (j = 0; j < ncell; j++) {
        if (psrng[j] == 0.0) {
            ...
            trace(NULL, 2, "msm fine pseudorange ext overflow %s psrng=%.3f\n", time_str(rtcm->time, 0), psrng[j]);
            psrng_val = -524288;
        } else {
...
static int encode_msm_phrng(rtcm_t* rtcm, int i, const double* phrng, int ncell) {
    ...
    for (j = 0; j < ncell; j++) {
        if (phrng[j] == 0.0) {
            ...
            trace(NULL, 2, "msm fine phase-range overflow %s phrng=%.3f\n", time_str(rtcm->time, 0), phrng[j]);
            phrng_val = -2097152;
        } else {
...
static int encode_msm_phrng_ex(rtcm_t* rtcm, int i, const double* phrng, int ncell) {
    ...
    for (j = 0; j < ncell; j++) {
        if (phrng[j] == 0.0) {
            ...
            trace(NULL, 2, "msm fine phase-range ext overflow %s phrng=%.3f\n", time_str(rtcm->time, 0), phrng[j]);
            phrng_val = -8388608;
        } else {
...
static int encode_msm_rate(rtcm_t* rtcm, int i, const double* rate, int ncell) {
    ...
    for (j = 0; j < ncell; j++) {
        if (rate[j] == 0.0) {
            ...
            trace(NULL, 2, "msm fine phase-range-rate overflow %s rate=%.3f\n", time_str(rtcm->time, 0), rate[j]);
            rate_val = -16384;
        } else {
...
static int encode_msm1(rtcm_t* rtcm, int sys, int sync) {
    ...
    trace(NULL, 3, "encode_msm1: sys=%d sync=%d\n", sys, sync);

    /* encode msm header */
    if (!(i = encode_msm_head(1, rtcm, sys, sync, &nsat, &ncell, rrng, rrate, NULL, psrng, NULL, NULL, NULL, NULL,
                              NULL))) {
        return 0;
    }
...
static int encode_msm2(rtcm_t* rtcm, int sys, int sync) {
    ...
    trace(NULL, 3, "encode_msm2: sys=%d sync=%d\n", sys, sync);

    /* encode msm header */
    if (!(i = encode_msm_head(2, rtcm, sys, sync, &nsat, &ncell, rrng, rrate, NULL, NULL, phrng, NULL, lock, half,
                              NULL))) {
        return 0;
    }
    /* encode msm satellite data */
...
static int encode_msm3(rtcm_t* rtcm, int sys, int sync) {
    ...
    trace(NULL, 3, "encode_msm3: sys=%d sync=%d\n", sys, sync);

    /* encode msm header */
    if (!(i = encode_msm_head(3, rtcm, sys, sync, &nsat, &ncell, rrng, rrate, NULL, psrng, phrng, NULL, lock, half,
                              NULL))) {
        return 0;
    }
    /* encode msm satellite data */
...
static int encode_msm4(rtcm_t* rtcm, int sys, int sync) {
    ...
    trace(NULL, 3, "encode_msm4: sys=%d sync=%d\n", sys, sync);

    /* encode msm header */
    if (!(i = encode_msm_head(4, rtcm, sys, sync, &nsat, &ncell, rrng, rrate, NULL, psrng, phrng, NULL, lock, half,
                              cnr))) {
        return 0;
    }
    /* encode msm satellite data */
...
static int encode_msm5(rtcm_t* rtcm, int sys, int sync) {
    ...
    trace(NULL, 3, "encode_msm5: sys=%d sync=%d\n", sys, sync);

    /* encode msm header */
    if (!(i = encode_msm_head(5, rtcm, sys, sync, &nsat, &ncell, rrng, rrate, info, psrng, phrng, rate, lock, half,
                              cnr))) {
        return 0;
    }
    /* encode msm satellite data */
...
static int encode_msm6(rtcm_t* rtcm, int sys, int sync) {
    ...
    trace(NULL, 3, "encode_msm6: sys=%d sync=%d\n", sys, sync);

    /* encode msm header */
    if (!(i = encode_msm_head(6, rtcm, sys, sync, &nsat, &ncell, rrng, rrate, NULL, psrng, phrng, NULL, lock, half,
                              cnr))) {
        return 0;
    }
    /* encode msm satellite data */
...
static int encode_msm7(rtcm_t* rtcm, int sys, int sync) {
    ...
    trace(NULL, 3, "encode_msm7: sys=%d sync=%d\n", sys, sync);

    /* encode msm header */
    if (!(i = encode_msm_head(7, rtcm, sys, sync, &nsat, &ncell, rrng, rrate, info, psrng, phrng, rate, lock, half,
                              cnr))) {
        return 0;
    }
    /* encode msm satellite data */
...
static int encode_type1230(rtcm_t* rtcm, int sync) {
    ...
    trace(NULL, 3, "encode_type1230: sync=%d\n", sync);
...
int encode_rtcm3(rtcm_t* rtcm, int type, int subtype, int sync) {
    ...
    trace(NULL, 3, "encode_rtcm3: type=%d subtype=%d sync=%d\n", type, subtype, sync);
    ...
    switch (type) {
        ...
        case 4076:
            ...
            break;
        case 2001:
            ret = encode_lcltrop(rtcm, type);
            break; /* tentative */
        case 2002:
            ret = encode_lcliono(rtcm, type);
            break; /* tentative */
        case 2003:
            ret = encode_lcliono(rtcm, type);
            break; /* tentative */
