...
const char* msm_sig_cmp[32] = {
    /* BeiDou: ref [17] table 3.5-108; B2b (7D/7P/7Z, IDs 25-27) per RTCM 10403.3 Amendment 2 (#333) */
    "", "2I", "2Q", "2X", "", "",   "",   "6I", "6Q", "6X", "",   "", "", "7I", "7Q", "7X",
    "", "",   "",   "",   "", "5D", "5P", "5X", "7D", "7P", "7Z", "", "", "1D", "1P", "1X"};
const char* msm_sig_irn[32] = {
    /* NavIC/IRNSS: ref [17] table 3.5-108.3 */
    "", "", "", "", "", "",   "", "", "", "", "", "", "", "", "", "",
    "", "", "", "", "", "5A", "", "", "", "", "", "", "", "", "", ""};
/* SSR signal and tracking mode IDs ------------------------------------------*/
const uint8_t ssr_sig_gps[32] = {CODE_L1C, CODE_L1P, CODE_L1W, CODE_L1Y, CODE_L1M, CODE_L2C,
                                 CODE_L2D, CODE_L2S, CODE_L2L, CODE_L2X, CODE_L2P, CODE_L2W,
                                 CODE_L2Y, CODE_L2M, CODE_L5I, CODE_L5Q, CODE_L5X};
const uint8_t ssr_sig_glo[32] = {CODE_L1C, CODE_L1P, CODE_L2C, CODE_L2P, CODE_L4A,
                                 CODE_L4B, CODE_L6A, CODE_L6B, CODE_L3I, CODE_L3Q};
const uint8_t ssr_sig_gal[32] = {CODE_L1A, CODE_L1B, CODE_L1C, CODE_L1X, CODE_L1Z, CODE_L5I, CODE_L5Q,
                                 CODE_L5X, CODE_L7I, CODE_L7Q, CODE_L7X, CODE_L8I, CODE_L8Q, CODE_L8X,
                                 CODE_L6A, CODE_L6B, CODE_L6C, CODE_L6X, CODE_L6Z};
const uint8_t ssr_sig_qzs[32] = {CODE_L1C, CODE_L1S, CODE_L1L, CODE_L2S, CODE_L2L, CODE_L2X, CODE_L5I,
                                 CODE_L5Q, CODE_L5X, CODE_L6S, CODE_L6L, CODE_L6X, CODE_L1X};
const uint8_t ssr_sig_cmp[32] = {CODE_L2I, CODE_L2Q, CODE_L2X, CODE_L7I, CODE_L7Q, CODE_L7X, CODE_L6I, CODE_L6Q,
...
/* S/N ratio -----------------------------------------------------------------*/
static uint16_t snratio(double snr) { return (uint16_t)(snr <= 0.0 || 100.0 <= snr ? 0.0 : snr / SNR_UNIT + 0.5); }
...
static int test_staid(rtcm_t* rtcm, int staid) {
    ...
    if (rtcm->staid == 0 || rtcm->obsflag) {
        ...
        trace(NULL, 2, "rtcm3 %d staid invalid id=%d %d\n", type, staid, rtcm->staid);
...
static int decode_head1001(rtcm_t* rtcm, int* sync) {
    ...
    trace(NULL, 4, "decode_head1001: time=%s nsat=%d sync=%d\n", tstr, nsat, *sync);
...
static int decode_type1002(rtcm_t* rtcm) {
    ...
    for (j = 0; j < nsat && rtcm->obs.n < MAXOBS && i + 74 <= rtcm->len * 8; j++) {
        ...
        if (!(sat = satno(sys, prn))) {
            trace(NULL, 2, "rtcm3 1002 satellite number error: prn=%d\n", prn);
            continue;
        }
...
static int decode_type1004(rtcm_t* rtcm) {
    ...
    for (j = 0; j < nsat && rtcm->obs.n < MAXOBS && i + 125 <= rtcm->len * 8; j++) {
        ...
        if (!(sat = satno(sys, prn))) {
            trace(NULL, 2, "rtcm3 1004 satellite number error: sys=%d prn=%d\n", sys, prn);
            continue;
        }
        tt = timediff(rtcm->obs.data[0].time, rtcm->time);
        if (rtcm->obsflag || fabs(tt) > 1E-9) {
...
static int decode_type1005(rtcm_t* rtcm) {
    ...
    if (i + 140 == rtcm->len * 8) {
        ...
        trace(NULL, 2, "rtcm3 1005 length error: len=%d\n", rtcm->len);
        return -1;
    }
    if (rtcm->outtype) {
        msg = rtcm->msgtype + strlen(rtcm->msgtype);
...
static int decode_type1006(rtcm_t* rtcm) {
    ...
    if (i + 156 <= rtcm->len * 8) {
        ...
        trace(NULL, 2, "rtcm3 1006 length error: len=%d\n", rtcm->len);
        return -1;
    }
    if (rtcm->outtype) {
        msg = rtcm->msgtype + strlen(rtcm->msgtype);
        ...
        ecef2pos(re, pos);
        sprintf(msg, " staid=%4d pos=%.8f %.8f %.3f anth=%.3f", staid, pos[0] * R2D, pos[1] * R2D, pos[2],
                anth * 0.0001);
    }
    /* test station id */
    if (!test_staid(rtcm, staid)) {
...
static int decode_type1007(rtcm_t* rtcm) {
    ...
    if (i + 28 + 8 * n <= rtcm->len * 8) {
        ...
        trace(NULL, 2, "rtcm3 1007 length error: len=%d\n", rtcm->len);
        return -1;
    }
    if (rtcm->outtype) {
        msg = rtcm->msgtype + strlen(rtcm->msgtype);
...
static int decode_type1008(rtcm_t* rtcm) {
    ...
    if (i + 36 + 8 * (n + m) <= rtcm->len * 8) {
        ...
        trace(NULL, 2, "rtcm3 1008 length error: len=%d\n", rtcm->len);
        return -1;
    }
    if (rtcm->outtype) {
        msg = rtcm->msgtype + strlen(rtcm->msgtype);
...
static int decode_head1009(rtcm_t* rtcm, int* sync) {
    ...
    time2str(rtcm->time, tstr, 2);
    trace(NULL, 4, "decode_head1009: time=%s nsat=%d sync=%d\n", tstr, nsat, *sync);
...
static int decode_type1010(rtcm_t* rtcm) {
    ...
    for (j = 0; j < nsat && rtcm->obs.n < MAXOBS && i + 79 <= rtcm->len * 8; j++) {
        ...
        i += 8;
        if (!(sat = satno(sys, prn))) {
            trace(NULL, 2, "rtcm3 1010 satellite number error: prn=%d\n", prn);
            continue;
        }
...
static int decode_type1012(rtcm_t* rtcm) {
    ...
    for (j = 0; j < nsat && rtcm->obs.n < MAXOBS && i + 130 <= rtcm->len * 8; j++) {
        ...
        i += 8;
        if (!(sat = satno(sys, prn))) {
            trace(NULL, 2, "rtcm3 1012 satellite number error: sys=%d prn=%d\n", sys, prn);
            continue;
        }
        if (!rtcm->nav.glo_fcn[prn - 1]) {
            rtcm->nav.glo_fcn[prn - 1] = fcn - 7 + 8; /* fcn+8 */
...
static int decode_type1019(rtcm_t* rtcm) {
    ...
    if (i + 476 <= rtcm->len * 8) {
        ...
        trace(NULL, 2, "rtcm3 1019 length error: len=%d\n", rtcm->len);
        return -1;
    }
    if (prn >= 40) {
        sys = SYS_SBS;
    ...
    trace(NULL, 4, "decode_type1019: prn=%d iode=%d toe=%.0f\n", prn, eph.iode, eph.toes);

    if (rtcm->outtype) {
        msg = rtcm->msgtype + strlen(rtcm->msgtype);
        sprintf(msg, " prn=%2d iode=%3d iodc=%3d week=%d toe=%6.0f toc=%6.0f svh=%02X", prn, eph.iode, eph.iodc, week,
                eph.toes, toc, eph.svh);
    }
    if (!(sat = satno(sys, prn))) {
        trace(NULL, 2, "rtcm3 1019 satellite number error: prn=%d\n", prn);
        return -1;
    }
    eph.sat = sat;
...
static int decode_type1020(rtcm_t* rtcm) {
    ...
    char* msg;
    int i = 24 + 12, prn, sat, week, tb, Bn, Cn, Cn_a, P1, P2, P3, P4, P, M, ln, sys = SYS_GLO;
    ...
    if (i + 348 <= rtcm->len * 8) {
        ...
        i += 5;
        Cn = getbitu(rtcm->buff, i, 1);
        i += 1;
        Cn_a = getbitu(rtcm->buff, i, 1);
        i += 1;
        P1 = getbitu(rtcm->buff, i, 2);
        i += 2;
        ...
        i += 5;
        P3 = getbitu(rtcm->buff, i, 1);
        i += 1;
        geph.gamn = getbitg(rtcm->buff, i, 11) * P2_40;
        i += 11;
        P = getbitu(rtcm->buff, i, 2);
        i += 2;
        ln = getbitu(rtcm->buff, i, 1);
        i += 1;
        ...
        i += 5;
        P4 = getbitu(rtcm->buff, i, 1);
        i += 1;
        geph.sva = getbitu(rtcm->buff, i, 4);
        i += 4 + 11;
        M = getbitu(rtcm->buff, i, 2);
    } else {
        trace(NULL, 2, "rtcm3 1020 length error: len=%d\n", rtcm->len);
        return -1;
    }
    if (!(sat = satno(sys, prn))) {
        trace(NULL, 2, "rtcm3 1020 satellite number error: prn=%d\n", prn);
        return -1;
    }
    trace(NULL, 4, "decode_type1020: prn=%d tk=%02.0f:%02.0f:%02.0f\n", prn, tk_h, tk_m, tk_s);

    if (rtcm->outtype) {
        msg = rtcm->msgtype + strlen(rtcm->msgtype);
        sprintf(msg, " prn=%2d tk=%02.0f:%02.0f:%02.0f frq=%2d Bn=%d tb=%d", prn, tk_h, tk_m, tk_s, geph.frq, Bn, tb);
    }
    geph.sat = sat;
    ...
    if (!strstr(rtcm->opt, "-EPHALL")) {
        if (fabs(timediff(geph.toe, rtcm->nav.geph[prn - 1].toe)) < 1.0 && geph.svh == rtcm->nav.geph[prn - 1].svh) {
            return 0; /* unchanged */
        }
    }
...
static int decode_type1029(rtcm_t* rtcm) {
    ...
    if (i + 60 <= rtcm->len * 8) {
        ...
        trace(NULL, 2, "rtcm3 1029 length error: len=%d\n", rtcm->len);
        return -1;
    }
    if (i + nchar * 8 > rtcm->len * 8) {
        trace(NULL, 2, "rtcm3 1029 length error: len=%d nchar=%d\n", rtcm->len, nchar);
        return -1;
    }
...
static int decode_type1033(rtcm_t* rtcm) {
    ...
    if (i + 60 + 8 * (n + m + n1 + n2 + n3) <= rtcm->len * 8) {
        ...
        trace(NULL, 2, "rtcm3 1033 length error: len=%d\n", rtcm->len);
        return -1;
    }
    if (rtcm->outtype) {
        msg = rtcm->msgtype + strlen(rtcm->msgtype);
...
static int decode_type1041(rtcm_t* rtcm) {
    ...
    if (i + 482 - 12 <= rtcm->len * 8) {
        ...
        trace(NULL, 2, "rtcm3 1041 length error: len=%d\n", rtcm->len);
        return -1;
    }
    trace(NULL, 4, "decode_type1041: prn=%d iode=%d toe=%.0f\n", prn, eph.iode, eph.toes);

    if (rtcm->outtype) {
        msg = rtcm->msgtype + strlen(rtcm->msgtype);
        sprintf(msg, " prn=%2d iode=%3d week=%d toe=%6.0f toc=%6.0f svh=%02X", prn, eph.iode, week, eph.toes, toc,
                eph.svh);
    }
    if (!(sat = satno(sys, prn))) {
        trace(NULL, 2, "rtcm3 1041 satellite number error: prn=%d\n", prn);
        return -1;
    }
    eph.sat = sat;
    eph.type = 0; /* ephemeris type = LNAV */
...
static int decode_type1044(rtcm_t* rtcm) {
    ...
    if (i + 473 <= rtcm->len * 8) {
        ...
        trace(NULL, 2, "rtcm3 1044 length error: len=%d\n", rtcm->len);
        return -1;
    }
    trace(NULL, 4, "decode_type1044: prn=%d iode=%d toe=%.0f\n", prn, eph.iode, eph.toes);

    if (rtcm->outtype) {
        msg = rtcm->msgtype + strlen(rtcm->msgtype);
        sprintf(msg, " prn=%3d iode=%3d iodc=%3d week=%d toe=%6.0f toc=%6.0f svh=%02X", prn, eph.iode, eph.iodc, week,
                eph.toes, toc, eph.svh);
    }
    if (!(sat = satno(sys, prn))) {
        trace(NULL, 2, "rtcm3 1044 satellite number error: prn=%d\n", prn);
        return -1;
    }
    eph.sat = sat;
    eph.type = 0; /* ephemeris type = LNAV */
    ...
    if (!strstr(rtcm->opt, "-EPHALL")) {
        if (eph.iode == rtcm->nav.eph[sat - 1].iode && eph.iodc == rtcm->nav.eph[sat - 1].iodc) {
            return 0; /* unchanged */
        }
    }
    rtcm->nav.eph[sat - 1] = eph;
...
static int decode_type1045(rtcm_t* rtcm) {
    ...
    if (i + 484 <= rtcm->len * 8) {
        ...
        trace(NULL, 2, "rtcm3 1045 length error: len=%d\n", rtcm->len);
        return -1;
    }
    trace(NULL, 4, "decode_type1045: prn=%d iode=%d toe=%.0f\n", prn, eph.iode, eph.toes);

    if (rtcm->outtype) {
        msg = rtcm->msgtype + strlen(rtcm->msgtype);
        sprintf(msg, " prn=%2d iode=%3d week=%d toe=%6.0f toc=%6.0f hs=%d dvs=%d", prn, eph.iode, week, eph.toes, toc,
                e5a_hs, e5a_dvs);
    }
    if (!(sat = satno(sys, prn))) {
        trace(NULL, 2, "rtcm3 1045 satellite number error: prn=%d\n", prn);
        return -1;
    }
    if (strstr(rtcm->opt, "-GALINAV")) {
        return 0;
    ...
    eph.sat = sat;
    eph.type = 1;           /* ephemeris type = F/NAV */
    ...
    eph.iodc = eph.iode;
    if (!strstr(rtcm->opt, "-EPHALL")) {
        if (eph.iode == rtcm->nav.eph[sat - 1 + MAXSAT].iode) {
            return 0; /* unchanged */
        }
    }
...
static int decode_type1046(rtcm_t* rtcm) {
    ...
    if (i + 492 <= rtcm->len * 8) {
        ...
        trace(NULL, 2, "rtcm3 1046 length error: len=%d\n", rtcm->len);
        return -1;
    }
    trace(NULL, 4, "decode_type1046: prn=%d iode=%d toe=%.0f\n", prn, eph.iode, eph.toes);

    if (rtcm->outtype) {
        msg = rtcm->msgtype + strlen(rtcm->msgtype);
        sprintf(msg, " prn=%2d iode=%3d week=%d toe=%6.0f toc=%6.0f hs=%d %d dvs=%d %d", prn, eph.iode, week, eph.toes,
                toc, e5b_hs, e1_hs, e5b_dvs, e1_dvs);
    }
    if (!(sat = satno(sys, prn))) {
        trace(NULL, 2, "rtcm3 1046 satellite number error: prn=%d\n", prn);
        return -1;
    }
    if (strstr(rtcm->opt, "-GALFNAV")) {
        return 0;
...
static int decode_type1042(rtcm_t* rtcm) {
    ...
    if (i + 499 <= rtcm->len * 8) {
        ...
        trace(NULL, 2, "rtcm3 1042 length error: len=%d\n", rtcm->len);
        return -1;
    }
    trace(NULL, 4, "decode_type1042: prn=%d iode=%d toe=%.0f\n", prn, eph.iode, eph.toes);
    ...
    if (!(sat = satno(sys, prn))) {
        trace(NULL, 2, "rtcm3 1042 satellite number error: prn=%d\n", prn);
        return -1;
    }
    eph.sat = sat;
    ...
    eph.A = sqrtA * sqrtA;
    if (!strstr(rtcm->opt, "-EPHALL")) {
        if (timediff(eph.toe, rtcm->nav.eph[sat - 1].toe) == 0.0 && eph.iode == rtcm->nav.eph[sat - 1].iode &&
...
static int decode_ssr1_head(rtcm_t* rtcm, int sys, int subtype, int* sync, int* iod, double* udint, int* refd,
                            int* hsize) {
    char *msg, tstr[64];
...
static int decode_ssr2_head(rtcm_t* rtcm, int sys, int subtype, int* sync, int* iod, double* udint, int* hsize) {
    char *msg, tstr[64];
    int i = 24 + 12, nsat, udi, provid = 0, solid = 0, ns;
...
static int decode_ssr1(rtcm_t* rtcm, int sys, int subtype) {
    ...
    for (j = 0; j < nsat && i + 121 + np + ni + nj <= rtcm->len * 8; j++) {
        ...
        if (!(sat = satno(sys, prn))) {
            trace(NULL, 2, "rtcm3 %d satellite number error: prn=%d\n", type, prn);
            continue;
        }
...
static int decode_ssr7_head(rtcm_t* rtcm, int sys, int subtype, int* sync, int* iod, double* udint, int* dispe, int* mw,
                            int* hsize) {
    char *msg, tstr[64];
    int i = 24 + 12, nsat, udi, provid = 0, solid = 0, ns;
...
static int decode_ssr7(rtcm_t* rtcm, int sys, int subtype) {
    ...
    if ((nsat = decode_ssr7_head(rtcm, sys, subtype, &sync, &iod, &udint, &dispe, &mw, &i)) < 0) {
        trace(NULL, 2, "rtcm3 %d length error: len=%d\n", type, rtcm->len);
        return -1;
    }
    switch (sys) {
...
static void sigindex(int sys, const uint8_t* code, int n, const char* opt, int* idx) {
    ...
    for (i = nex = 0; i < n; i++) {
        if (ex[i] == 0) {
            ...
            trace(NULL, 2, "rtcm msm: no space in obs data sys=%d code=%d\n", sys, code[i]);
            idx[i] = -1;
        }
#if 0 /* for debug */
        trace(NULL,2,"sig pos: sys=%d code=%d ex=%d idx=%d\n",sys,code[i],ex[i],idx[i]);
#endif
    }
}
/* save obs data in MSM message ----------------------------------------------*/
static void save_msm_obs(rtcm_t* rtcm, int sys, msm_h_t* h, const double* r, const double* pr, const double* cp,
    ...
    for (i = 0; i < h->nsig; i++) {
        ...
        if (code[i] != CODE_NONE) {
            ...
            trace(NULL, 2, "rtcm3 %d: unknown signal id=%2d\n", type, h->sigs[i]);
        }
    }
    trace(NULL, 3, "rtcm3 %d: signals=%s\n", type, msm_type);
    ...
    for (i = j = 0; i < h->nsat; i++) {
        ...
        if ((sat = satno(sys, prn))) {
            ...
            trace(NULL, 2, "rtcm3 %d satellite error: prn=%d\n", type, prn);
        }
        fcn = 0;
        if (sys == SYS_GLO) {
        ...
        for (k = 0; k < h->nsig; k++) {
            ...
            if (sat && index >= 0 && idx[k] >= 0) {
                ...
                rtcm->obs.data[index].LLI[idx[k]] = lossoflock(rtcm, sat, idx[k], lock[j]) + (half[j] ? 3 : 0);
...
static int decode_msm_head(rtcm_t* rtcm, int sys, int* sync, int* iod, msm_h_t* h, int* hsize) {
    ...
    *h = h0;
    ...
    if (h->nsat * h->nsig > 64) {
        trace(NULL, 2, "rtcm3 %d number of sats and sigs error: nsat=%d nsig=%d\n", type, h->nsat, h->nsig);
        return -1;
    }
    if (i + h->nsat * h->nsig > rtcm->len * 8) {
        trace(NULL, 2, "rtcm3 %d length error: len=%d nsat=%d nsig=%d\n", type, rtcm->len, h->nsat, h->nsig);
        return -1;
    }
    ...
    *hsize = i;
    ...
    time2str(rtcm->time, tstr, 2);
    trace(NULL, 4, "decode_head_msm: time=%s sys=%d staid=%d nsat=%d nsig=%d sync=%d iod=%d ncell=%d\n", tstr, sys,
          staid, h->nsat, h->nsig, *sync, *iod, ncell);

    if (rtcm->outtype) {
        msg = rtcm->msgtype + strlen(rtcm->msgtype);
        sprintf(msg, " staid=%4d %s nsat=%2d nsig=%2d iod=%2d ncell=%2d sync=%d", staid, tstr, h->nsat, h->nsig, *iod,
                ncell, *sync);
    }
    return ncell;
}
...
static int decode_msm4(rtcm_t* rtcm, int sys) {
    ...
    if (i + h.nsat * 18 + ncell * 48 > rtcm->len * 8) {
        trace(NULL, 2, "rtcm3 %d length error: nsat=%d ncell=%d len=%d\n", type, h.nsat, ncell, rtcm->len);
        return -1;
    }
    for (j = 0; j < h.nsat; j++) {
        r[j] = 0.0;
...
static int decode_type1230(rtcm_t* rtcm) {
    ...
    if (i + 20 >= rtcm->len * 8) {
        trace(NULL, 2, "rtcm3 1230: length error len=%d\n", rtcm->len);
        return -1;
    }
    staid = getbitu(rtcm->buff, i, 12);
    i += 12;
    ...
    if (rtcm->outtype) {
        sprintf(rtcm->msgtype + strlen(rtcm->msgtype), " staid=%4d align=%d mask=0x%X", staid, align, mask);
    }
    /* test station ID */
    if (!test_staid(rtcm, staid)) {
        return -1;
...
static int decode_type4073(rtcm_t* rtcm) {
    ...
    trace(NULL, 2, "rtcm3 4073: unsupported message subtype=%d\n", subtype);
    return 0;
}
...
static int decode_type4076(rtcm_t* rtcm) {
    ...
    if (i + 3 + 8 >= rtcm->len * 8) {
        trace(NULL, 2, "rtcm3 4076: length error len=%d\n", rtcm->len);
        return -1;
    }
    ...
    if (rtcm->outtype) {
        sprintf(rtcm->msgtype + strlen(rtcm->msgtype), " ver=%d subtype=%3d", ver, subtype);
    }
    switch (subtype) {
        case 21:
    ...
    trace(NULL, 2, "rtcm3 4076: unsupported message subtype=%d\n", subtype);
    return 0;
}

/* decode type 4050: QZSS LEX signal raw data MAGELLAN SYSTEMS JAPAN, INC. -----------------*/
static int decode_type4050(rtcm_t* rtcm) {
    int i = 24 + 12, tow, errbits, prn, type, alert;
    uint8_t tmpbuff[1200]; /* message buffer */

    if (1776 != rtcm->len * 8) {
        trace(NULL, 2, "rtcm3 4050: length error len=%d\n", rtcm->len);
        return -1;
    }
    ...
    tow = getbitu(rtcm->buff, i, 20);
    i += 20; /* TOW */
    errbits = getbitu(rtcm->buff, i, 4);
    i += 4; /* Number of correction error bits */
    prn = getbitu(rtcm->buff, i, 8);
    i += 8; /* PRN */
    type = getbits(rtcm->buff, i, 8);
    i += 8; /* L6 Message Type ID */
    alert = getbits(rtcm->buff, i, 1);
    i += 1; /* Alert Flag */

    trace(NULL, 3, "rtcm3 4050: tow=%d errbits=%d prn=%d type=%d alert=%d\n", tow, errbits, prn, type, alert);

    memcpy(tmpbuff, rtcm->buff, sizeof(tmpbuff));
    ...
    rtcm->buff[2] = 0xFC;
    rtcm->buff[3] = 0x1D;
    memcpy(rtcm->buff + 4, tmpbuff + 8, sizeof(uint8_t) * 217);
...
int decode_rtcm3(rtcm_t* rtcm) {
    ...
    trace(NULL, 3, "decode_rtcm3: len=%3d type=%d\n", rtcm->len, type);
    ...
    switch (type) {
        ...
        case 1265:
            ...
            break; /* draft */
        case 1266:
            ret = decode_ssr7(rtcm, SYS_GLO, 0);
            break; /* draft */
        case 1267:
        ...
        case 4076:
            ...
            break;
        case 4050:
            ret = decode_type4050(rtcm);
            break;
        case 2001:
            ret = decode_lcltrop(rtcm, type);
            break; /* tentative */
        case 2002:
            ret = decode_lcliono(rtcm, type);
            break; /* tentative */
        case 2003:
            ret = decode_lcliono(rtcm, type);
            break; /* tentative */
