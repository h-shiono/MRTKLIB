...
static void outrpos(FILE* fp, const double* r, const solopt_t* opt) {
    ...
    if (opt->posf == SOLF_LLH || opt->posf == SOLF_ENU) {
        ...
        if (opt->degf) {
            ...
            fprintf(fp, "%3.0f%s%02.0f%s%08.5f%s%4.0f%s%02.0f%s%08.5f%s%10.4f", dms1[0], sep, dms1[1], sep, dms1[2],
                    sep, dms2[0], sep, dms2[1], sep, dms2[2], sep, pos[2]);
        } else {
...
/* output header -------------------------------------------------------------*/
static void outheader(FILE* fp, char** file, int n, const prcopt_t* popt, const solopt_t* sopt) {
...
/* process positioning -------------------------------------------------------*/
static void procpos(mrtk_ctx_t* ctx, FILE* fp, const prcopt_t* popt, const solopt_t* sopt, int mode) {
    gtime_t time = {0};
    sol_t sol = {{0}};
    rtk_t rtk;
...
static int valcomb(const sol_t* solf, const sol_t* solb) {
    ...
    for (i = 0; i < 3; i++) {
        ...
        trace(NULL, 2, "degrade fix to float: %s dr=%.3f %.3f %.3f std=%.3f %.3f %.3f\n", tstr + 11, dr[0], dr[1],
              dr[2], SQRT(var[0]), SQRT(var[1]), SQRT(var[2]));
        return 0;
    }
    return 1;
}
...
static void readpreceph(char** infile, int n, const prcopt_t* prcopt, nav_t* nav, sbs_t* sbs) {
    ...
    fp_rtcm = NULL;
    qzssl6e_file[0] = qzssl6e_path[0] = '\0';
    fp_qzssl6e = NULL;
    for (i = 0; i < MIONO_MAX_PRN; i++) {
        qzssl6d_file[i][0] = qzssl6d_path[i][0] = '\0';
        fp_qzssl6d[i] = NULL;
    }
    for (i = 0; i < CLAS_CH_NUM; i++) {
        clas_file[i][0] = clas_path[i][0] = '\0';
        fp_clas[i] = NULL;
    }
    stat_file[0] = cstat_file[0] = '\0';
    fp_stat = NULL;
    ...
    /* MADOCA-PPP L6 and CLAS L6 files: discriminate by PRN and mode */
    {
        ...
        for (i = 0; i < n; i++) {
            if ((ext = strrchr(infile[i], '.')) && (!strcmp(ext, ".l6") || !strcmp(ext, ".L6"))) {
                /* L6D (PRN=200,201) — always detected */
                if ((ext - infile[i] >= 4) && (!strcmp(ext - 4, ".200.l6") || !strcmp(ext - 4, ".200.L6") ||
    ...
    for (i = 0; i < n; i++) {
        if ((ext = strrchr(infile[i], '.')) && (!strcmp(ext, ".stat") || !strcmp(ext, ".STAT"))) {
...
static int readobsnav(gtime_t ts, gtime_t te, double ti, char** infile, const int* index, int n, const prcopt_t* prcopt,
...
static int avepos(mrtk_ctx_t* ctx, double* ra, int rcv, const obs_t* obs, const nav_t* nav, const prcopt_t* opt) {
    obsd_t data[MAXOBS];
    gtime_t ts = {0};
    sol_t sol = {{0}};
    int i, j, n = 0, m, iobs;
...
static int antpos(mrtk_ctx_t* ctx, prcopt_t* opt, int rcvno, const obs_t* obs, const nav_t* nav, const sta_t* sta,
                  const char* posfile) {
...
static int openses(mrtk_ctx_t* ctx, const prcopt_t* popt, const solopt_t* sopt, const filopt_t* fopt, nav_t* nav,
...
static int execses(mrtk_ctx_t* ctx, gtime_t ts, gtime_t te, double ti, const prcopt_t* popt, const solopt_t* sopt,
                   const filopt_t* fopt, int flag, char** infile, const int* index, int n, char* outfile) {
    FILE* fp;
    prcopt_t popt_ = *popt;
    ...
    if (clas_ctx) {
        ...
        if (popt_.isb == ISBOPT_TABLE) {
            setisb(&navs, popt_.rectype[0], popt_.rectype[1], &navs.stas[0], &navs.stas[1]);
        }
    }
    ...
    } else if ((PMODE_DGPS <= popt_.mode && popt_.mode <= PMODE_STATIC) || popt_.mode == PMODE_VRS_RTK) {
...
static int execses_r(mrtk_ctx_t* ctx, gtime_t ts, gtime_t te, double ti, const prcopt_t* popt, const solopt_t* sopt,
                     const filopt_t* fopt, int flag, char** infile, const int* index, int n, char* outfile,
                     const char* rov) {
    gtime_t t0 = {0};
    int i, stat = 0;
...
static int execses_b(mrtk_ctx_t* ctx, gtime_t ts, gtime_t te, double ti, const prcopt_t* popt, const solopt_t* sopt,
                     const filopt_t* fopt, int flag, char** infile, const int* index, int n, char* outfile,
 ...
 *-----------------------------------------------------------------------------*/
extern int postpos(mrtk_ctx_t* ctx, gtime_t ts, gtime_t te, double ti, double tu, const prcopt_t* popt,
                   const solopt_t* sopt, const filopt_t* fopt, char** infile, int n, char* outfile, const char* rov,
                   const char* base) {
    ...
    if (ts.time != 0 && te.time != 0 && tu >= 0.0) {
        ...
        for (i = 0;; i++) { /* for each periods */
            ...
            for (j = k = nf = 0; j < n; j++) {
                ...
                if (ext &&
                    (!strcmp(ext, ".rtcm3") || !strcmp(ext, ".RTCM3") || !strcmp(ext, ".l6") || !strcmp(ext, ".L6"))) {
            ...
            /* execute processing session */
            stat = execses_b(ctx, tts, tte, ti, popt, sopt, fopt, flag, ifile, index, nf, ofile, rov, base);
        ...
        /* execute processing session */
        stat = execses_b(ctx, ts, te, ti, popt, sopt, fopt, 1, ifile, index, n, ofile, rov, base);
        ...
        /* execute processing session */
        stat = execses_b(ctx, ts, te, ti, popt, sopt, fopt, 1, infile, index, n, outfile, rov, base);
    }
