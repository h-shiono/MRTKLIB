...
extern void sunmoonpos(gtime_t tut, const double* erpv, double* rsun, double* rmoon, double* gmst);
...
extern void matmul(const char* tr, int n, int k, int m, double alpha, const double* A, const double* B, double beta,
                   double* C);
...
extern int iontec(gtime_t time, const nav_t* nav, const double* pos, const double* azel, int opt, double* delay,
                  double* var);
extern int sbsioncorr(gtime_t time, const nav_t* nav, const double* pos, const double* azel, double* delay,
                      double* var);
...
extern void satposs(gtime_t teph, const obsd_t* obs, int n, const nav_t* nav, int ephopt, double* rs, double* dts,
                    double* var, int* svh);
...
extern int const_iono_corr(rtk_t* rtk, const obsd_t* obs, const nav_t* nav, const int n, const double* azel,
                           const int* exc, const double* rr, const double* x, double* v, double* H, double* var);
...
extern int pppoutstat(rtk_t* rtk, char* buff) {
    ...
    /* receiver clocks */
    i = IC(0, &rtk->opt);
    p += sprintf(p, "$CLK,%d,%.3f,%d,%d,%.3f,%.3f,%.3f,%.3f\n", week, tow, rtk->sol.stat, 1, x[i] * 1E9 / CLIGHT,
                 x[i + 1] * 1E9 / CLIGHT, STD(rtk, i) * 1E9 / CLIGHT, STD(rtk, i + 1) * 1E9 / CLIGHT);
...
static double mwmeas(const obsd_t* obs, const nav_t* nav, int f) {
    ...
    if (freq1 == 0.0 || freq2 == 0.0 || obs->L[0] == 0.0 || obs->L[f] == 0.0 || obs->P[0] == 0.0 || obs->P[f] == 0.0) {
        return 0.0;
    }
...
static void detslp_ll(rtk_t* rtk, const obsd_t* obs, int n) {
    ...
    for (i = 0; i < n && i < MAXOBS; i++) {
        for (j = 0; j < rtk->opt.nf && j < NFREQ; j++) {
            ...
            trace(NULL, 3, "detslp_ll: slip detected %s sat=%2d f=%d\n", satid, obs[i].sat, j + 1);
...
static void detslp_gf(rtk_t* rtk, const obsd_t* obs, int n, const nav_t* nav) {
    ...
    for (i = 0; i < n && i < MAXOBS; i++) {
        for (f = 1; f < rtk->opt.nf && f < NFREQ; f++) {
            ...
            trace(NULL, 4, "detslip_gf: sat=%2d f=%d gf0=%8.3f gf1=%8.3f\n", obs[i].sat, f + 1, g0, g1);

            if (g0 != 0.0 && fabs(g1 - g0) > rtk->opt.thresslip) {
                satno2id(obs[i].sat, satid);
                trace(NULL, 2, "detslip_gf: slip detected %s sat=%2d f=%d gf=%8.3f->%8.3f\n", satid, obs[i].sat, f + 1,
                      g0, g1);
...
static void detslp_mw(rtk_t* rtk, const obsd_t* obs, int n, const nav_t* nav) {
    ...
    for (i = 0; i < n && i < MAXOBS; i++) {
        for (f = 1; f < rtk->opt.nf && f < NFREQ; f++) {
            ...
            trace(NULL, 4, "detslip_mw: sat=%2d f=%d mw0=%8.3f mw1=%8.3f\n", obs[i].sat, f + 1, w0, w1);

            if (w0 != 0.0 && fabs(w1 - w0) > THRES_MW_JUMP) {
                satno2id(obs[i].sat, satid);
                trace(NULL, 2, "detslip_mw: slip detected %s sat=%2d f=%d mw=%8.3f->%8.3f\n", satid, obs[i].sat, f + 1,
                      w0, w1);
...
static void detslp_ssr(rtk_t* rtk, const obsd_t* obs, int n, const nav_t* nav) {
    ...
    for (i = 0; i < n && i < MAXOBS; i++) {
        if (!nav->ssr_ch[0][obs[i].sat - 1].t0[5].time) {
            continue;
        }
        ...
        for (j = 0; j < rtk->opt.nf && j < NFREQ; j++) {
            ...
            if (nav->ssr_ch[0][obs[i].sat - 1].pbias[ssrcode - 1] == 0.0) {
                continue;
            }
            ...
            if (discont0 != discont1) {
                satno2id(obs[i].sat, satid);
                trace(NULL, 2,
                      "detslp_ssr: discont detected sys=%2d %s sat=%2d f=%d code=%2d discont0=%d discont1=%d\n", sys,
                      satid, obs[i].sat, j + 1, ssrcode, discont0, discont1);
...
static void udtrop_ppp(rtk_t* rtk) {
    ...
    if (rtk->x[i] == 0.0) {
        ...
        trace(NULL, 2, "udtrop_ppp: rtk->opt.prn[2]=%.3e\n", rtk->opt.prn[2]);
        if (rtk->opt.tropopt >= TROPOPT_ESTG) {
            for (j = i + 1; j < i + 3; j++) {
...
static void udiono_ppp(rtk_t* rtk, const obsd_t* obs, int n, const nav_t* nav) {
    ...
    for (i = 0; i < MAXSAT; i++) {
        ...
        if (rtk->x[j] != 0.0 && (int)rtk->ssat[i].outc[0] > gap_resion) {
            ...
            if ((int)rtk->ssat[i].outc[0] == gap_resion + 1 && rtk->ssat[i].azel[1] >= rtk->opt.elmin) {
                satno2id(i + 1, satid);
                el = rtk->ssat[i].azel[1] * R2D;
                trace(NULL, 3, "udiono_ppp(%4d): %s reset ionospheric estimates. %s outc=%d el=%2.0f\n", __LINE__,
    ...
    for (i = 0; i < n; i++) {
        ...
        if (rtk->x[j] == 0.0) {
            ...
            if (azel[1] >= rtk->opt.elmin) {
                satno2id(obs[i].sat, satid);
                trace(NULL, 3, "udiono_ppp(%4d): %s initialize ionospheric estimates. %s el=%2.0f x=%8.4f\n", __LINE__,
            ...
            if (L[0] == 0.0 || L[1] == 0.0 || freq1 == 0.0 || freq2 == 0.0 || slip[0] || slip[1]) {
                rtk->ssat[obs[i].sat - 1].ionc = 0.0;
                continue;
            }
...
static void udbias_ppp(rtk_t* rtk, const obsd_t* obs, int n, const nav_t* nav) {
    ...
    for (f = 0; f < NF(&rtk->opt); f++) {
        ...
        for (i = k = 0; i < n && i < MAXOBS; i++) {
            sat = obs[i].sat;
            j = IB(sat, f, &rtk->opt);
            corr_meas(obs + i, nav, rtk->ssat[sat - 1].azel, &rtk->opt, dantr, dants, 0.0, L, P, &Lc, &Pc, 0);
            ...
            if (rtk->opt.ionoopt == IONOOPT_IFLC) {
                ...
                if (obs[i].P[0] == 0.0 || obs[i].P[1] == 0.0 || obs[i].P[f] == 0.0 || freq1 == 0.0 || freq2 == 0.0 ||
                    freq3 == 0.0) {
                    continue;
                }
...
static int ppp_res(int post, const obsd_t* obs, int n, const double* rs, const double* dts, const double* var_rs,
                   const int* svh, const double* dr, int* exc, const nav_t* nav, const double* x, const double* cov,
                   rtk_t* rtk, double* v, double* H, double* R, double* azel, double* resrms) {
    prcopt_t* opt = &rtk->opt;
    double y, r, cdtr, bias, C = 0.0, rr[3], pos[3], e[3], dtdx[3] = {0.0}, L[NFREQ], P[NFREQ], Lc, Pc;
    double var[MAXOBS * 2 * NFREQ], dtrp = 0.0, dion = 0.0, vart = 0.0, vari = 0.0, freq, ifb, ifb_std, bias_std,
                                    dion_std, dtrp_std;
    ...
    for (i = 0; i < n && i < MAXOBS; i++) {
        ...
        for (j = 0; j < 2 * NF(opt); j++) {
            ...
            /* variance */
            var[nv] = varerr(obs[i].sat, sys, azel[1 + i * 2], j / 2, j % 2, opt) + vart + SQR(C) * vari +
                      opt->uraratio * var_rs[i];
            if (sys == SYS_GLO && j % 2 == 1) {
                var[nv] += VAR_GLO_IFB;
            }

            trace(NULL, post > 0 ? 3 : 4,
                  "ppp_res: post=%d %s %s sat=%3d %s%s res=%9.4f sig=%9.4f el=%4.1f cdtr=%7.4f "
                  "C=%7.4f dion=%7.4f (%5.2f) dtrp=%7.4f (%5.2f) bias=%9.4f (%5.2f) ifb=%9.4f (%5.2f) fix=%d\n",
                  post, str, satstr, sat, j % 2 ? "C" : "L", code2obs(obs[i].code[j / 2]), v[nv], sqrt(var[nv]),
                  azel[1 + i * 2] * R2D, cdtr, C, dion, dion_std, dtrp, dtrp_std, bias, bias_std, ifb, ifb_std,
    ...
    resrms[1] = resc_rms;
    trace(NULL, 2,
          "ppp_res: post=%d, %s, nresp=, %2d, resp_rms=, %7.3f, resp_ave=, %7.3f, nresc=, %2d, resc_rms=, %7.3f, "
          "resc_ave=, %7.3f\n",
    ...
    if (post && ne > 0) {
        ...
        sat = obs[maxobs].sat;
        satno2id(sat, satstr);
        trace(NULL, 2, "outlier (%d) rejected %s %s sat=%2d %s%d res=%9.4f el=%4.1f\n", post, str, satstr, sat,
...
static int test_hold_amb(rtk_t* rtk) {
    ...
    for (i = 0; i < MAXSAT; i++) {
        if (rtk->ssat[i].fix[0] != 2) {
            continue;
        }
        for (j = 0; j < MAXSAT; j++) {
            if (rtk->ssat[j].fix[0] != 2) {
                continue;
            }
...
extern void pppos(mrtk_ctx_t* ctx, rtk_t* rtk, const obsd_t* obs, int n, const nav_t* nav) {
    ...
    time2str(obs[0].time, str, 2);
    trace(NULL, 3, "pppos   : time=%s nx=%d n=%d\n", str, rtk->nx, n);
    ...
    for (i = 0; i < MAX_ITER; i++) {
        ...
        if (!(nv = ppp_res(0, obs, n, rs, dts, var, svh, dr, exc, nav, xp, Pp, rtk, v, H, R, azel, resrms))) {
            trace(NULL, 2, "%s ppp (%d) no valid obs data\n", str, i + 1);
            break;
        }
        /* measurement update of ekf states */
        if ((info = filter(xp, Pp, H, v, R, rtk->nx, nv))) {
            trace(NULL, 2, "%s ppp (%d) filter error info=%d\n", str, i + 1, info);
            break;
        }
        /* postfit residuals */
    ...
    if (i >= MAX_ITER) {
        trace(NULL, 2, "%s ppp (%d) iteration overflows\n", str, i);
    }
    if (stat == SOLQ_PPP) {
        /* ambiguity resolution in ppp */
        if ((ret = ppp_ar(ctx, rtk, obs, n, exc, nav, azel, xp, Pp)) &&
            ...
            if (ret > 1) {
                stat = SOLQ_FIX; /* narrow-lane fixed */
            } else {
                stat = SOLQ_FIX_WL; /* only wide-lane fixed */
            }

            trace(NULL, 2,
                  "pppos(%4d): time=%s stat=%d std=%.3f resp=%7.3f -> %7.3f (%6.3f) resc=%7.3f -> %7.3f (%6.3f)\n",
                  __LINE__, str, stat, norm(std, 3), resrms[0], resrms_ar[0], resrms_ar[0] / resrms[0], resrms[1],
                  resrms_ar[1], resrms_ar[1] / resrms[1]);
        } else {
            rtk->nfix = 0;
        }
        /* update solution status */
        ...
        if (stat == SOLQ_FIX && test_hold_amb(rtk)) {
            matcpy(rtk->x, xp, rtk->nx, 1);
            matcpy(rtk->P, Pp, rtk->nx, rtk->nx);
            trace(NULL, 3, "pppos(%4d): %s hold ambiguity nfix=%4d / %4d\n", __LINE__, str, rtk->nfix, rtk->opt.minfix);
        }
    }
    free(rs);
    free(dts);
