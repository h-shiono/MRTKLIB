...
static void update_antpos(rtksvr_t* svr, int index) {
    ...
    if (svr->rtk.opt.refpos == POSOPT_RTCM && index == 1) {
        ...
        if (sta->deltype) {     /* xyz */
            del[2] = -sta->hgt; /* marker = ARP - ant-height */
            enu2ecef(pos, del, dr);
            for (i = 0; i < 3; i++) {
...
static void update_ssr(rtksvr_t* svr, int index) {
    int i, sys, prn, iode;
    int ephset = 0; /* augmentation target is Galileo I/NAV ref [1] */
    ...
    for (i = 0; i < MAXSAT; i++) {
        ...
        if (sys == SYS_GPS || sys == SYS_GAL || sys == SYS_QZS) {
            if (svr->nav.eph[i + MAXSAT * ephset].iode != iode &&
                svr->nav.eph[i + MAXSAT * (2 + ephset)].iode != iode) {
                continue;
            }
        } else if (sys == SYS_GLO) {
            if (svr->nav.geph[prn - 1].iode != iode && svr->nav.geph[prn - 1 + MAXPRNGLO].iode != iode) {
                continue;
            }
        }
...
static void update_lcl(rtksvr_t* svr, int index) {
    if (block2stat(&svr->rtcm[index], &svr->nav.stat) == 0) {
        return;
    }
...
static void update_svr(rtksvr_t* svr, int ret, obs_t* obs, nav_t* nav, int ephsat, int ephset, sbsmsg_t* sbsmsg,
                       int index, int iobs) {
    tracet(NULL, 4, "updatesvr: ret=%d ephsat=%d ephset=%d index=%d\n", ret, ephsat, ephset, index);
        ...
        if (svr->format[index] == STRFMT_RTCM2) {
            ...
            if (svr->clas) {
                ...
                if (ret == 10) {
                    ...
                    if (net > 0) {
                        int rc =
                            clas_bank_get_close(svr->clas, svr->clas->l6buf[ch].time, net, ch, &svr->clas->current[ch]);
                        if (rc == 0) {
                            clas_update_global(&svr->nav, &svr->clas->current[ch], ch);
                            clas_check_grid_status(svr->clas, &svr->clas->current[ch], ch);
                        }
                        ...
                        if (tmp) {
                            ...
                            for (net2 = 1; net2 < CLAS_MAX_NETWORK; net2++) {
                                if (clas_bank_get_close(svr->clas, svr->clas->l6buf[ch].time, net2, ch, tmp) == 0) {
                                    nfound++;
...
static void send_nmea(rtksvr_t* svr, uint32_t* tickreset) {
    ...
    if (svr->nmeareq == 1) { /* lat-lon-hgt mode */
        ...
        if (bl >= svr->bl_reset && (int)(tick - *tickreset) > MIN_INT_RESET) {
            ...
            tracet(NULL, 2, "send reset: bl=%.3f rr=%.3f %.3f %.3f rb=%.3f %.3f %.3f\n", bl, svr->rtk.sol.rr[0],
                   svr->rtk.sol.rr[1], svr->rtk.sol.rr[2], svr->rtk.rb[0], svr->rtk.rb[1], svr->rtk.rb[2]);
            *tickreset = tick;
        }
        if (norm(svr->rtk.sol.rr, 3) <= 0.0) {
            return;
    ...
    for (cycle = 0; svr->state; cycle++) {
        ...
        if (fobs[1] > 0 && svr->rtk.opt.refpos == POSOPT_SINGLE) {
            if ((svr->rtk.opt.maxaveep <= 0 || svr->nave < svr->rtk.opt.maxaveep) &&
                pntpos(ctx, svr->obs[1][0].data, svr->obs[1][0].n, &svr->nav, &svr->rtk.opt, &sol, NULL, NULL, msg)) {
                svr->nave++;
                for (i = 0; i < 3; i++) {
        ...
        for (i = 0; i < fobs[0]; i++) { /* for each rover observation data */
            ...
            rtkpos(ctx, &svr->rtk, obs.data, obs.n, &svr->nav);
            {
                int ii;
                for (ii = 0; ii < 6; ii++) {
                    svr->rtk.prev_qr[ii] = svr->rtk.sol.qr[ii];
                }
            }
            rtksvrunlock(svr);
...
extern int rtksvrinit(rtksvr_t* svr) {
    ...
    if (!(svr->nav.eph = (eph_t*)malloc(sizeof(eph_t) * MAXSAT * 4)) ||
        ...
        tracet(NULL, 1, "rtksvrinit: malloc error\n");
        return 0;
    }
 ...
 *          gtime_t rst      I  raw/rtcm data start time
 *          char   *errmsg   O  error message
 * return : status (1:ok 0:error)
 *-----------------------------------------------------------------------------*/
extern int rtksvrstart(rtksvr_t* svr, int cycle, int buffsize, int* strs, char** paths, int* formats, int navsel,
                       char** cmds, char** cmds_periodic, char** rcvopts, int nmeacycle, int nmeareq,
                       const double* nmeapos, prcopt_t* prcopt, solopt_t* solopt, stream_t* moni, gtime_t rst,
                       char* errmsg) {
    gtime_t time, time0 = {0};
    int i, j, rw;

    tracet(NULL, 3, "rtksvrstart: cycle=%d buffsize=%d navsel=%d nmeacycle=%d nmeareq=%d\n", cycle, buffsize, navsel,
           nmeacycle, nmeareq);
    ...
    for (i = 0; i < 3; i++) { /* input/log streams */
        svr->nb[i] = svr->npb[i] = 0;
        if (!(svr->buff[i] = (uint8_t*)malloc(buffsize)) || !(svr->pbuf[i] = (uint8_t*)malloc(buffsize))) {
            tracet(NULL, 1, "rtksvrstart: malloc error\n");
    ...
    for (i = 0; i < 8; i++) {
        ...
        if (i < 3) {
            ...
            if (strs[i] == STR_FILE) {
                svr->raw[i].time = strgettime(svr->stream + i);
                svr->rtcm[i].time = strgettime(svr->stream + i);
            } else {
                svr->raw[i].time = time;
 ...
 * return : status (1:ok 0:error)
 *-----------------------------------------------------------------------------*/
extern int rtksvropenstr(rtksvr_t* svr, int index, int str, const char* path, const solopt_t* solopt) {
    tracet(NULL, 3, "rtksvropenstr: index=%d str=%d path=%s\n", index, str, path);
    ...
    if (!stropen(svr->stream + index, str, STR_MODE_W, path)) {
        tracet(NULL, 2, "stream open error: index=%d\n", index);
        rtksvrunlock(svr);
        return 0;
    }
    if (index <= 4) {
...
extern void rtksvrclosestr(rtksvr_t* svr, int index) {
    tracet(NULL, 3, "rtksvrclosestr: index=%d\n", index);
 ...
 *-----------------------------------------------------------------------------*/
extern int rtksvrostat(rtksvr_t* svr, int rcv, gtime_t* time, int* sat, double* az, double* el, int** snr, int* vsat) {
    int i, j, ns;

    tracet(NULL, 4, "rtksvrostat: rcv=%d\n", rcv);
...
extern int rtksvrmark(rtksvr_t* svr, const char* name, const char* comment) {
    ...
    tracet(NULL, 4, "rtksvrmark:name=%s comment=%s\n", name, comment);
    ...
    for (i = 0; i < 2; i++) {
        p = buff;
        if (svr->solopt[i].posf == SOLF_STAT) {
            p += sprintf(p, "$MARK,%d,%.3f,%d,%.4f,%.4f,%.4f,%s,%s\r\n", week, tow, svr->rtk.sol.stat,
                         svr->rtk.sol.rr[0], svr->rtk.sol.rr[1], svr->rtk.sol.rr[2], name, comment);
        } else if (svr->solopt[i].posf == SOLF_NMEA) {
            p += sprintf(p, "$GPTXT,01,01,02,MARK:%s,%s,%.9f,%.9f,%.4f,%d,%s", name, tstr, pos[0] * R2D, pos[1] * R2D,
                         pos[2], svr->rtk.sol.stat, comment);
            for (q = (char*)buff + 1, sum = 0; *q; q++) {
                sum ^= *q; /* check-sum */
            }
            p += sprintf(p, "*%02X\r\n", sum);
        } else {
            p += sprintf(p, "%s MARK: %s,%s,%.9f,%.9f,%.4f,%d,%s\r\n", COMMENTH, name, tstr, pos[0] * R2D, pos[1] * R2D,
                         pos[2], svr->rtk.sol.stat, comment);
        }
