...
static int is_obsmsg(int msg) {
    return (1001 <= msg && msg <= 1004) || (1009 <= msg && msg <= 1012) || (1071 <= msg && msg <= 1077) ||
           (1081 <= msg && msg <= 1087) || (1091 <= msg && msg <= 1097) || (1101 <= msg && msg <= 1107) ||
           (1111 <= msg && msg <= 1117) || (1121 <= msg && msg <= 1127) || (1131 <= msg && msg <= 1137);
}
/* test navigation data message ----------------------------------------------*/
static int is_navmsg(int msg) {
    return msg == 1019 || msg == 1020 || msg == 1044 || msg == 1045 || msg == 1046 || msg == 1042 || msg == 63 ||
           msg == 1041;
}
 ...
 *-----------------------------------------------------------------------------*/
extern strconv_t* strconvnew(int itype, int otype, const char* msgs, int staid, int stasel, const char* opt) {
    strconv_t* conv;
    double tint;
    char buff[1024], *p;
    int msg;
...
extern void strsvrinit(strsvr_t* svr, int nout) {
    ...
    tracet(NULL, 3, "strsvrinit: nout=%d\n", nout);
 ...
 *-----------------------------------------------------------------------------*/
extern int strsvrstart(strsvr_t* svr, int* opts, int* strs, char** paths, char** logs, strconv_t** conv, char** cmds,
                       char** cmds_periodic, const double* nmeapos) {
    int i, rw, stropt[5] = {0};
    char file1[MAXSTRPATH], file2[MAXSTRPATH], *p;

    tracet(NULL, 3, "strsvrstart:\n");
    ...
    if (!(svr->buff = (uint8_t*)malloc(svr->buffsize)) || !(svr->pbuf = (uint8_t*)malloc(svr->buffsize))) {
        free(svr->buff);
        free(svr->pbuf);
        return 0;
    }
